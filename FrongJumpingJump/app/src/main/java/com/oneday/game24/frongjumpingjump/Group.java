...
public boolean TouchEvent(MotionEvent event) 
{
    ...
    void DrawCommonBg(GL10 gl)
    {
    	...
    	if(Counter%79==1 && ScreenReceiver.wasScreenOn)
      	  M.FrogSound(GameRenderer.mContext,R.raw.frog);
...
//	      DrawTextureSS(gl,mGR.mTex_Google,-.819f,-.399f-mGR.mTex_Google.Height()*.4f,1,mGR.mSel==4?1.2f:mGR.mAni,0,mGR.mTex_Google.Height()*.4f);
	      for(int i=0;i<mGR.mTex_Menu.length && i<2 && mGR.mAni==1f;i++)
	         DrawTransScal(gl,mGR.mTex_Menu[i],0,.139f-i*mGR.mMenuAni*1.8f,mGR.mSel==i+1?1.2f:1,mGR.mSel==i+1?.5f:1);
	...
	boolean HandleMenu(MotionEvent event)
	{
		mGR.mSel=0;
		for(int i=0;i<mGR.mTex_Menu.length && i<2 && mGR.mAni==1f;i++)
  		  if(CircRectsOverlap(0,.139f-i*mGR.mMenuAni*1.8f,mGR.mTex_Menu[0].width()*.4f,mGR.mTex_Menu[0].Height()*.3f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
		  {
  			 mGR.mSel=i+1; //Menu
		  } 
		...
		if(event.getAction() == MotionEvent.ACTION_UP)
		{
			switch(mGR.mSel)
			{
			   ...
			   case 3:
					...
					mIntent.setData(Uri.parse("https://play.google.com/store/apps/details?id=com.oneday.game24.frongjumpingjump"));
...
//		DrawTexture(gl,mGR.mTex_SplashFrog[ani[Counter2%ani.length]],.509f,-.479f+Math.abs((float)Math.sin(y)*.02f));
	...
	boolean HandleWorld(MotionEvent event)
	{
		...
		if(event.getAction() == MotionEvent.ACTION_MOVE && dx>.1f /*&& mGR.mSel==0*/)
		{
			...
			{
				M.SwingSound(GameRenderer.mContext,R.raw.swing);
	...
	void gamelogic() 
	{
		...
		if(mGR.mCharAni< 8)
		{
			...
			if(mGR.mCharAni == 8)
			{
				...
				{
					M.DumpingSound(GameRenderer.mContext,R.raw.dumping);
					mGR.move = -.03f;
					mGR.mCharAni = 0;
					mGR.st--;
					if(mGR.st < 0)
	...
	void DrawGamePlay(GL10 gl)
	{
		...
		for(int i = 0; i < mGR.mStep.length; i++) 
		{
			if(mGR.mStep[i].x>-1.2f && mGR.mStep[i].x<1.2f)
			{
				...
				else if(mGR.jumpCount<mGR.mTex_WaterAni.length && mGR.mStep[i].x >mGR.plyX -.05f && mGR.mStep[i].x<mGR.plyX +.25f)
				{
					if(mGR.jumpCount<2)
					...
					{
						 M.GameOver(GameRenderer.mContext,R.raw.gameover);
						if(M.Vibrate)
		...
		if(mGR.jumpCount>=mGR.mTex_WaterAni.length)
		{
			...
			{
			  DrawTexture(gl,mGR.mTex_Frog[mGR.mCharAni],mGR.plyX,mGR.plyY);
			}
			else
			{
				float y= (float)Math.toRadians((mGR.mStep[mGR.st].moveCnt)%360);
				DrawTexture(gl,mGR.mTex_Frog[0],mGR.plyX,(mGR.plyY -Math.abs((float)Math.sin(y)*.025f))+hilana);
