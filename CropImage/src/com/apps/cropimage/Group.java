...
public class Group extends Mesh {
	...
	public void Draw_Game(GL10 gl){
		...
		for(int i =0;i<GameRenderer.mEditImg.size();i++){
			GameRenderer.mEditImg.get(i).tex.drawAll(gl, GameRenderer.mEditImg.get(i).x, GameRenderer.mEditImg.get(i).y, 
					GameRenderer.mEditImg.get(i).ang + GameRenderer.mEditImg.get(i).langle,
		...
		if (GameRenderer.mEditImg.size() > 0) {
			...
			case 2:
				...
				case 30:
					if (mGR.mMode == 0) {
						GameRenderer.mEditImg.get(yimg).ang += 1f;
					} else if (mGR.mGellary.size() > 0) {
		...
		if(GameRenderer.mEditImg.size()<2) {
			if(GameRenderer.mEditImg.size()==0 || mGR.showPopup) {
				...
				DrawTransScal(gl, mGR.mTex_Play, 0,0.65f,sel == 1 ?1.1f:1,sel == 1 ?0.5f:1);
			}
		}
	...
	boolean Handle_Crop(MotionEvent event) {
		...
		if(event.getAction() == MotionEvent.ACTION_DOWN && GameRenderer.mEditImg.size() < 2){
			...
			if(GameRenderer.mEditImg.size() == 1){
				if(CircRectsOverlap(0 ,0.65f, mGR.mTex_Play.width()*.5f, mGR.mTex_Play.Height()*.5f, s2wX(event.getX()), s2wY(event.getY()), .02f)){
					mGR.showPopup =true;
					sel =1;
				}
			}
		...
		if(GameRenderer.mEditImg.size() > 0 && event.getAction() == MotionEvent.ACTION_DOWN){
			...
			case 2:
				...
				case 32:
					if (mGR.mMode == 0) {
						GameRenderer.mEditImg.get(yimg).ang += 90f;
					} else if (mGR.mGellary.size() > 0) {
...
//    	int work = yimg;
		...
		case MotionEvent.ACTION_UP: // first finger lifted
			 ...
			 GameRenderer.mEditImg.get(yimg).ang +=GameRenderer.mEditImg.get(yimg).langle;
			 GameRenderer.mEditImg.get(yimg).langle = 0;
			break;
		...
		case MotionEvent.ACTION_MOVE:
			if(mode == 1)
			{
				...
				if (last4 == 2){
					...
					GameRenderer.mEditImg.get(yimg).langle = (float)Math.toDegrees(oldAng-newAng);
				}
