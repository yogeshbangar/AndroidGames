...
//				Achivment
	...
	void Draw_Shop(GL10 gl)
	{
		...
		DrawTextureS(gl, mGR.mTex_Reward, 0, -.70f,mGR.mSel == 6?1.2f:sx);
	}
	boolean Handle_Shop(MotionEvent event)
	{
		...
		if(CircRectsOverlap( .00f,-.7f, mGR.mTex_Buy.width()*.9f, mGR.mTex_Buy.Height()*.9f, screen2worldX(event.getX()), screen2worldY(event.getY()), .02f))
		{
			mGR.mSel = 6;
		}
		if(MotionEvent.ACTION_UP == event.getAction()){
...
//						setBlastAnim(mGR.mOpp[i].x,mGR.mOpp[i].y);
						...
						M.sound7(GameRenderer.mContext, R.raw.coin_truck);
					}
					else{
						if(mGR.mPlayer.life >0){
//							mGR.mBlast.set(mGR.mOpp[i].x,mGR.mOpp[i].y,0);
