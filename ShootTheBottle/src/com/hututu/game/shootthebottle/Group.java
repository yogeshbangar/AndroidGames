...
//				GameRenderer.mStart.Submitscore(R.string.leaderboard_high_score);
	...
	void DrawMenu(GL10 gl)
	{
		...
		if(mGR.mSel!=6)
			DrawTexture(gl, mGR.mTex_Exit[0]	, 0, -.95f);
		...
		case 6:
			if(menuAni<5)
				DrawTexture(gl, mGR.mTex_Exit[1]	, 0, -.95f);
			else
				DrawTexture(gl, mGR.mTex_Exit[2]	, 0, -.95f);
			break;
		}
	}
		...
		for(i =0;i<mGR.mStone.length;i++)
		{
			if(mGR.mStone[i].checkInside())
			{
				for(int j =0;j<mGR.mBottle.length;j++)
				{
					...
					{
						if(CircRectsOverlap(mGR.mBottle[j].x, mGR.mBottle[j].y, mGR.mTex_Bottle[0].width()/4, mGR.mTex_Bottle[0].Height()/4  
...
//			DrawTexture(gl, mGR.mTex_Shadow, mGR.mBottle[i].x, mGR.mBottle[i].y-sy);
			if(mGR.mBottle[i].isBlast)
			{
				...
				DrawTexture(gl, mGR.mTex_Bottle[mGR.mBottle[i].type], mGR.mBottle[i].x, mGR.mBottle[i].y);
			if(mGR.mBottle[i].Sheld>0)
			{
