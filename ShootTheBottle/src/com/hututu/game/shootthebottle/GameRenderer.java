...
public class GameRenderer implements Renderer 
{
	...
	SimplePlane[] mTex_Bottle,mTex_BottleD,mTex_Break,mTex_Stone,mTex_BG,mTex_Title;
	SimplePlane[] mTex_Particle,mTex_Font,mTex_Star,mTex_HighScore,mTex_Submit,mTex_FontB;
	SimplePlane[] mTex_Contine,mTex_Exit,mTex_Help,mTex_Info,mTex_Newgame,mTex_Nextlvl,/*mTex_Rateus,*/mTex_Retry,mTex_Menu;
...
//			mTex_Shadow 		= add("bottleshadow.png");
			...
			mTex_Exit			= new SimplePlane[3];
			mTex_Exit[0]		= add("menufont/exit0.png");
			mTex_Exit[1]		= add("menufont/exit1.png");
			mTex_Exit[2]		= add("menufont/exit2.png");
...
//			mTex_Rateus[2]		= add("menufont/rateus2.png");
			...
			mTex_Bottle			= new SimplePlane[18];
			Bitmap b 			= LoadImgfromAsset("bottlestrip0.png");
			for(i = 0;i<mTex_Bottle.length;i++)
				mTex_Bottle[i] 	= addBitmap(Bitmap.createBitmap(b, i*b.getWidth()/mTex_Bottle.length,0,b.getWidth()/mTex_Bottle.length,b.getHeight(),null, true));
			
			b 					= LoadImgfromAsset("bottlestrip2.png");
			mTex_Break			= new SimplePlane[18];
			for(i = 0;i<mTex_Break.length;i++)
				mTex_Break[i] 	= addBitmap(Bitmap.createBitmap(b, i*b.getWidth()/mTex_Bottle.length,0,b.getWidth()/mTex_Bottle.length,b.getHeight(),null, true));
	...
	void gameReset()
	{
		...
		for(i =0;i<mBottle.length;i++)
			mBottle[i].set(-.75f+(i*.3f), .38f,M.mRand.nextInt(mTex_Bottle.length));
		...
		isPause = false;
		root.scal = .5f;
		try{ads_handler.sendEmptyMessage(0);}catch(Exception e){}
	}
	
	private Handler ads_handler = new Handler() {public void handleMessage(Message msg) {mStart.loadInter();}};
