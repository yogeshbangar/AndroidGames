...
public class Level02 {
	...
	GameRenderer mGR;
	public Level02(GameRenderer _GameRenderer) {
		mGR =_GameRenderer;
		mTex_bg = GameRenderer.add("l2/bg2.jpg");
...
//		mTex_BgTile.drawScal(gl, 0, -.07f, 200, 1);
		Group.DrawTexture(gl, mTex_bg, Bx1, 0);
		Group.DrawTexture(gl, mTex_bg, Bx2, 0);
		for (int i = 0; i < mStep.length; i++) {
...
//						Group.DrawTexture(gl, mTex_Bubbul, mStep[i].x, -.7f);
				}else if(jumpCount<val && mStep[i].x > px-.05f && mStep[i].x<px+.25f){
//					if(jumpCount<2)
					{
						//Group.DrawTexture(gl, mTex_Frog[jumpCount+6], mStep[i].x,-.35f);
						mGR.mLOne.Draw_AniChar(gl,mStep[i].x,vy, (mGR.mLOne.ani/mGR.mLOne.FrogAnim.length)%3==0,.6f);
						vy -= .1f;
					}
//					Group.DrawTexture(gl, mTex_JumpAnim[jumpCount%val], mStep[i].x,-.35f);
					if(Counter%2==0)
						jumpCount++;
				}
				if (mStep[i].Watch)
...
//			else
			vy =- .35f;
				mGR.mLOne.Draw_AniChar(gl, px, py, (mGR.mLOne.ani/mGR.mLOne.FrogAnim.length)%3==0,.6f);
		}
	...
	boolean Handle_GamePlay(MotionEvent event) {
		...
		if (MotionEvent.ACTION_DOWN == event.getAction()) {
			...
			else if(mGR.timesUp < 5 && mGR.ClockCount < 5 && (jumpCount>=val)){
				performAction();
			}
			st %= mStep.length;
		}
