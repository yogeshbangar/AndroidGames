...
public class Level13 {
	...
	SimplePlane mTex_BG,mTex_Panguin,mTex_Shadow,mTex_Obj[];
	GameRenderer mGR;
	public Level13 (GameRenderer _GameRenderer){
		mGR = _GameRenderer;
		mTex_Obj		= new SimplePlane[5];
		for(int i =0;i<mTex_Obj.length;i++)
			mTex_Obj[i]	= GameRenderer.add("l13/"+i+".png");
		mTex_BG			= GameRenderer.add("l13/13.png");
	...
	{
		mThirteen.update();
		if(Group.CirCir(mThirteen.x, mThirteen.y, mTex_Panguin.width()*.9f, mObj.x,mObj.y,mTex_Obj[0].Height()*.5f))
		{
	...
	void Draw_GamePlay(GL10 gl)
	{
		...
		if(mObj.no==5)
			...
			Group.DrawTexture(gl, mTex_Obj[mObj.no], mObj.x, mObj.y);
