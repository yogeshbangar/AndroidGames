...
public class Level12 {
	...
	SimplePlane mTex_BG,mTex_Step,mTex_Net;/*mTex_Sky,mTex_Penguin[],mTex_PSplash[]*/;
	GameRenderer mGR;
	public Level12(GameRenderer _GameRenderer)
	{
		mGR =_GameRenderer;
		mTex_Net		= GameRenderer.add("l12/net_up.png");
		mTex_BG			= GameRenderer.add("l12/bg12.png");
...
//						mCandy[i].vx =.002f;
			...
			if(!mCandy[i].woter && mCandy[i].x > 0.85 && mCandy[i].x < 0.1)
			{
				mCandy[i].x=1.2f;
...
//				if(mCandy[i].y>=-.3f)
					Group.DrawTexture(gl, mGR.mLFour.mTex_Peng[counter%mGR.mLFour.mTex_Peng.length],mCandy[i].x,mCandy[i].y);
//				else
//					Group.DrawTransScal(gl, mGR.mLFour.mTex_Peng[counter%mGR.mLFour.mTex_Peng.length],mCandy[i].x,mCandy[i].y,1,.5f);
...
//					Group.DrawTransScal(gl, mGR.mLsix.mTex_Fruit[mCandy[i].candy],mCandy[i].x,mCandy[i].y,1,.5f);
		...
		if(mWatch.y > -2f)
			Group.DrawTransScal(gl, mGR.mLSelect.mTex_BWatch,mWatch.x,mWatch.y,1,/*mWatch.y>=-.3f?1:.5f*/1);
...
//				Group.DrawTexture(gl, mGR.mLTwo.mTex_JumpAnim[Woter[i].no], Woter[i].x,Woter[i].y);
		...
		Group.DrawTexture(gl, mTex_Net, .87f, .11f);
