...
public class Level04 {
	...
	SimplePlane mTex_Peng[],mTex_Center;//,mTex_Face[];//,mTex_Copter[],mTex_Object[];
	SimplePlane mTex_Ball,mTex_Left,mTex_Right,mTex_BG,/*mTex_Pengu[],*/mTex_Gross;
	GameRenderer mGR;
	public Level04(GameRenderer _GameRenderer)
	{
		mGR = _GameRenderer;
		
		Bitmap b		= GameRenderer.LoadImgfromAsset("l4/ballstrip.png");
		mTex_Peng		= new SimplePlane[16];
...
//		}
//		mTex_Line		= GameRenderer.add("l4/ground_strip.png");
		mTex_Center		= GameRenderer.add("l4/center_circle.png");
		mTex_Ball		= GameRenderer.add("l4/small_ball.png");
		mTex_Left		= GameRenderer.add("l4/move_left.png");
		mTex_Right		= GameRenderer.add("l4/move_right.png");
		mTex_Gross		= GameRenderer.add("l4/grass.png");
...
//		mTex_Object[6]	= GameRenderer.add("l4/stars.png");//6
	...
	void gameLogic()
	{
		...
		{
			if(mY[i]<-1.3f){
				mY[i] = mY[i==mY.length-1?0:i+1] + mTex_Gross.Height();
			}
		}
	...
	void Draw_GamePlay(GL10 gl)
	{
		Counter++;
		for(int i=0;i<mY.length;i++){
			Group.DrawTexture(gl, mTex_Gross, 0, mY[i]);
//			if(i==2)
//				Group.DrawTexture(gl, mTex_Center, 0, mY[i]);
//			else
//				Group.DrawTexture(gl, mTex_Line, 0, mY[i]);
		}
		Group.DrawTexture(gl, mTex_Center, 0, mY[2]);
...
//		else
			Group.DrawTexture(gl, mTex_Peng[Counter%mTex_Peng.length], Player.x, Player.y);
...
//		Group.DrawTexture(gl, mTex_Copter[Counter%2], x, y+.3f);
		Group.DrawTexture(gl, mTex_Peng[Counter%mTex_Peng.length], x, y);
//		if((Counter2/ani.length)%3==0)
