...
//	private final float[] mRGBA = new float[] { 1.0f, 1.0f, 1.0f, 1.0f };
	...
	public void drawRotet(GL10 gl,float _x,float _y,float ang,float sx,float sy,float rgb[]) 
	{
		gl.glPushMatrix();
		gl.glFrontFace(GL10.GL_CCW);
		gl.glEnable(GL10.GL_CULL_FACE);
		...
		if (mTextureId != -1 && mTextureBuffer != null) {
			...
			gl.glTexCoordPointer(2, GL10.GL_FLOAT, 0, mTextureBuffer);
			gl.glBindTexture(GL10.GL_TEXTURE_2D, mTextureId);
			gl.glColor4f((rgb[0] + 10f) / 20f, (rgb[1] + 10f )/ 20f, (rgb[2] + 10f) / 20f, 1);
		}
		gl.glTranslatef(_x,_y,-100);
	...
	public void drawDark(GL10 gl,float x, float y,float sx, float sy, float t)
    {
        gl.glPushMatrix();
        gl.glEnableClientState(GL10.GL_VERTEX_ARRAY);
        gl.glVertexPointer(3, GL10.GL_FLOAT, 0, mVerticesBuffer);
