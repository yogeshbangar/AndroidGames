...
public class LevelSelection {
	...
	SimplePlane mTex_Bar, mTex_BarS, mTex_BStar, mTex_BWatch, mTex_Pluse, mTex_Rankbar, mTex_RankbarBG;
	SimplePlane mTex_StingBar, mTex_ShopSadow, mTex_freegold , mTex_Reset , mTex_ButtonS, mTex_AdsFree;
	...
	public LevelSelection(GameRenderer _GameRenderer)
	{
		...
		mTex_HolderS	= GameRenderer.add("ls/level_box_sadow.png");
		...
		mTex_Rank	= new SimplePlane[10];
		for(int i=0;i<mTex_Rank.length;i++)
			mTex_Rank[i]	= GameRenderer.add("rank/r"+(i+1)+".png");
		
		Bitmap b		= GameRenderer.LoadImgfromAsset("rank/r_eye.png");
		mTex_RankEye	= new SimplePlane[6];
		for(int i=0;i<mTex_RankEye.length;i++)
		{
			mTex_RankEye[i]	= GameRenderer.addBitmap(Bitmap.createBitmap(b,i*b.getWidth()/8,0,b.getWidth()/8, b.getHeight(), null, true));
		}
		...
		b.recycle();
		mTex_AIcon	= new SimplePlane[52];
		b		= GameRenderer.LoadImgfromAsset("ls/small_icons.png");
		for(int i=0;i<mTex_AIcon.length;i++)
		{
			mTex_AIcon[i]	= GameRenderer.addBitmap(Bitmap.createBitmap(b, 
					(i%16)*b.getWidth()/16, (i/16)*b.getHeight()/4,
					b.getWidth()/16, b.getHeight()/4, null, true));
			
		}
		
		
		
		mTex_Buy	= new SimplePlane[7];
		for(int i=0;i<mTex_Buy.length;i++)
			mTex_Buy[i]	= GameRenderer.add("shop_icon/"+i+".png");
		
		
		mTex_Free	= new SimplePlane[10];
		for(int i=0;i<mTex_Free.length;i++)
			mTex_Free[i]	= GameRenderer.add("free_icon/"+i+".png");
...
//		mGR.TStar =99;
		...
		Group.DrawTransScal(gl, mTex_BarS	,-.7f,0.84f,mSel==1?1.1f:1,1);
		Group.DrawTransScal(gl, mTex_Bar	,-.7f,0.84f,mSel==1?1.1f:1,mSel==1?0.5f:1);
		Group.DrawTransScal(gl, mTex_BarS	,0.7f,0.84f,mSel==2?1.1f:1,1);
		...
		mGR.Draw(gl, mGR.TStar+" / 65" , 0, 0.60f,0.84f,0);
		
		
		for(int i = 0;i<13;i++){
			if((i-mGameSel+sx > -2) && (i-mGameSel+sx) < 2)
			{
				Group.DrawTexture(gl, mTex_HolderS, i-mGameSel+sx, 0.1f);
				Group.DrawTransScal(gl, mTex_Holder , i-mGameSel+sx, 0.1f,mSel==i+100?1.02f:1,mSel==i+100?0.5f:1);
				if(M.UNLOCK[i]<=mGR.TStar){
					Group.DrawTransScal(gl, mTex_LIcon[i] , i-mGameSel+sx, .23f,mSel==i+100?1.02f:1,mSel==i+100?0.5f:1);
				}
				else
				{
					Group.DrawTransScal(gl, mTex_Lock , i-mGameSel+sx, .23f,mSel==i+100?1.02f:1,mSel==i+100?0.5f:1);
					Group.DrawTextureS(gl, mTex_BStar , i-mGameSel+sx-.15f, -.11f,.5f);
					mGR.Draw(gl, M.UNLOCK[i]+" / 65",0, i-mGameSel+sx-.05f,-.11f,0);
				}
				for(int j=0;j<5;j++)
					if (M.SCORE[i][j] <= mGR.BestScore[i]){
						Group.DrawTexture(gl, mTex_SY, i-mGameSel+sx+(-.205f+.103f*j), -.29f);
					}
			}
		}
		...
		{
			if(mGameSel==i)
				Group.DrawTexture(gl, mTex_Dot0, -.48f+i*.08f, -.60f);
			else
				Group.DrawTexture(gl, mTex_Dot1, -.48f+i*.08f, -.60f);
		}
...
//			GameRenderer.mStart.loadInter();//SmartHandler();
//		}
//	};
	void LevelLoad()
	{
...
//			}
	...
	void Draw_Main(GL10 gl)
	{
		...
		Group.DrawTexture(gl, mTex_Rank[((int)(mGR.mRankFill/16))%10],0,-.1f);
		if((counter/mGR.mLOne.FrogAnim.length)%2==0)
			Group.DrawTexture(gl, mTex_RankEye[mGR.mLOne.FrogAnim[counter%mGR.mLOne.FrogAnim.length]],.03f,.07f-.1f);
		else
			Group.DrawTexture(gl, mTex_RankEye[0],.03f,.07f-.1f);
		
		Group.DrawTransScal(gl, mTex_BarS	,-.71f,0.84f,mSel==1?1.1f:1,1);
		Group.DrawTransScal(gl, mTex_BarS	,0.71f,0.84f,mSel==2?1.1f:1,1);
		...
		mGR.Draw(gl, "x "+mGR.Clock, 0, -.73f,0.84f,0);
		mGR.Draw(gl, mGR.TStar+" / 65" , 0, 0.60f,0.84f,0);
		mGR.Draw(gl, "Rank: "+(int)((mGR.mRankFill/16)+1)+"-"+
		GameRenderer.mStart.getString(frog.RANK_STR[(int)(mGR.mRankFill/16)]) , 0, 0.0f,-.450f,1);
		
		
		Group.DrawTexture(gl, mTex_RankbarBG, 0.0f,-.65f);
		mTex_RankbarFil.drawSS(gl,-.3f,-.665f, mGR.mRankFill%16, 1);
		Group.DrawTexture(gl, mTex_Rankbar	, 0.0f,-.65f);
...
//		Group.DrawTransScal(gl, mTex_ShopB		, 0.9f, -.84f,mSel==7?1.1f:1,mSel==7?0.5f:1);
	...
	boolean Handle_Main(MotionEvent event)
	{
		...
		if(Group.CircRectsOverlap(0.0,0.00f, .3f, .4f, Group.screen2worldX(event.getX()), Group.screen2worldY(event.getY()),.05f))
		{
			mSel =3;
		}
		if(Group.CircRectsOverlap(-.9f, -.84f, mTex_AchievB.width()*.4f, mTex_AchievB.Height()*.4f, Group.screen2worldX(event.getX()), Group.screen2worldY(event.getY()),.05f))
...
//					popUp = true;
	...
	void Draw_About(GL10 gl)
	{
		...
		mGR.Draw(gl, "Version 1.0.3" 				, 0, 0.05f,-.0f,1);
		mGR.Draw(gl, "www.hututugames.com" 		, 0, 0.05f,-.15f,1);
...
//				M.GameScreen = M.GAMEABOUT;
	...
	{
		Group.DrawTexture(gl, mTex_BG, 0, 0);
		mGR.Draw(gl, "Achievements: 50/50", 0, 0.05f, 0.8f, 1);
		
		if(((sx >0 && mPage==0)||(sx <0 && mPage==4))&&!isDown){
			float mm = sx >0 ? -2:2;
			sx+=mm*.1f;
			if(Math.abs(sx)<.2f){
				sx =0;
		...
		for(int i=0;i<2;i++)
		{
			for(int j=0;j<5;j++)
			{
				...
				if(M.ACHIVE[((i*5+j+mPage*10)/4)%13][(i*5+j+mPage*10)%4]){
		...
		if((sx >0 && mPage>0)||(sx <0 && mPage<4)){
			...
			for(int i=0;i<2;i++)
			{
				for(int j=0;j<5;j++)
				{
					...
					if(sx<0){
						if(M.ACHIVE[((i*5+j+(mPage+1)*10)/4)%13][(i*5+j+(mPage+1)*10)%4])
		...
		{
			if(mPage==i)
				Group.DrawTexture(gl, mTex_Dot0, -.17f+i*.09f, -.55f);
			else
				Group.DrawTexture(gl, mTex_Dot1, -.17f+i*.09f, -.55f);
		}
		Group.DrawTexture(gl, mTex_ButtonS		, -.9f, -.84f);
...
//		Group.DrawTransScal(gl, mTex_AchievB	, 0.9f, -.84f,mSel==3?1.1f:1,mSel==3?0.5f:1);
		if(cout<10)
		{
			...
			if(cout%4<2){
				int t = M.ACHIVE_VALUE[((mPage*10+cout)/4)%13][(mPage*10+cout)%4]-mGR.Total[mGameSel];
				if(t>0)
					mGR.Draw(gl, t+" left.", 0,0.0f,0.03f, 1);
			}
			Group.DrawTextureS(gl, mTex_NoAds  	,0.65f,.35f,.5f);
			Group.DrawTexture(gl, mTex_AchieveBox, 0.0f,-.25f);
			if(M.ACHIVE[((mPage*10+cout)/4)%13][(mPage*10+cout)%4])
...
//					popUp = true;
	...
	void Draw_BUY(GL10 gl)
	{
		Group.DrawTexture(gl, mTex_BG, 0, 0);
		for(int i =0;i<7;i++){
			Group.DrawTexture(gl, mTex_ShopSadow, sx + move -.7f+i* .42f+.02f,-.05f);
			Group.DrawTransScal(gl, mTex_Shop		, sx + move -.7f+i* .42f,0,mSel==(i+1)?1.1f:1,mSel==(i+1)?0.5f:1);
			Group.DrawTexture(gl, mTex_Buy[i]	, sx + move -.7f+i* .42f,0.04f);
			if(i == 0){
				mGR.Draw(gl, "Remove Ads", 1, sx + move -.68f+i* .42f,0.4f,1);
				mGR.Draw_Num(gl, "$0.99" , 0, sx + move -.77f+i* .42f,-.36f);
			}
			if(i == 1){
				mGR.Draw(gl, "Three Stars", 1, sx + move -.68f+i* .42f,0.4f,1);
				mGR.Draw_Num(gl, "$0.99" , 0, sx + move -.77f+i* .42f,-.36f);
				Group.DrawTextureS(gl, mTex_BStar,  sx + move -.76f+i* .42f,-.16f,.5f);
				mGR.Draw(gl, " x 3",0			 ,  sx + move -.70f+i* .42f,-.16f,0);
			}
			if(i == 2){
				mGR.Draw(gl, "Ten Clocks", 1, sx + move -.68f+i* .42f,0.4f,1);
				mGR.Draw_Num(gl, "$0.99" , 0, sx + move -.77f+i* .42f,-.36f);
				Group.DrawTextureS(gl, mTex_BWatch,  sx + move -.78f+i* .42f,-.16f,.5f);
				mGR.Draw(gl, " x 10",0			 ,  sx + move -.70f+i* .42f,-.16f,0);
			}
			if(i == 3){
				mGR.Draw(gl, "Gift", 1, sx + move -.68f+i* .42f,0.43f,1);
				mGR.Draw(gl, "0f Clocks", 1, sx + move -.68f+i* .42f,0.35f,1);
				mGR.Draw_Num(gl, "$1.99" , 0, sx + move -.77f+i* .42f,-.36f);
				Group.DrawTexture(gl, mTex_AdsFree, sx + move -.7f+i* .42f,-.52f);
				Group.DrawTextureS(gl, mTex_BWatch,  sx + move -.78f+i* .42f,-.16f,.5f);
				mGR.Draw(gl, " x 25",0			 ,  sx + move -.70f+i* .42f,-.16f,0);
			}
			if(i == 4){
				mGR.Draw(gl, "Treasure", 1, sx + move -.68f+i* .42f,0.43f,1);
				mGR.Draw(gl, "0f Clocks", 1, sx + move -.68f+i* .42f,0.35f,1);
				mGR.Draw_Num(gl, "$9.99" , 0, sx + move -.77f+i* .42f,-.36f);
				Group.DrawTexture(gl, mTex_AdsFree, sx + move -.7f+i* .42f,-.52f);
				Group.DrawTextureS(gl, mTex_BWatch,  sx + move -.80f+i* .42f,-.16f,.5f);
				mGR.Draw(gl, " x 150",0			  ,  sx + move -.73f+i* .42f,-.16f,0);
			}
			if(i == 5){
				mGR.Draw(gl, "Unlock all", 1, sx + move -.68f+i* .42f,0.43f,1);
				mGR.Draw(gl, "mini Games", 1, sx + move -.68f+i* .42f,0.35f,1);
				mGR.Draw_Num(gl, "$4.99" , 0, sx + move -.77f+i* .42f,-.36f);
				Group.DrawTexture(gl, mTex_AdsFree, sx + move -.7f+i* .42f,-.52f);
			}
			if(i == 6){
				mGR.Draw(gl, "Unlock all", 1, sx + move -.68f+i* .42f,0.4f,1);
				mGR.Draw_Num(gl, "$11.9" , 0, sx + move -.77f+i* .42f,-.36f);
				Group.DrawTexture(gl, mTex_AdsFree, sx + move -.7f+i* .42f,-.52f);
			}
		}
		Group.DrawTransScal(gl, mTex_BarS	,-.7f,0.84f,1,1);
		Group.DrawTransScal(gl, mTex_Bar	,-.7f,0.84f,1,1);
		Group.DrawTransScal(gl, mTex_BarS	,0.7f,0.84f,1,1);
		Group.DrawTransScal(gl, mTex_Bar	,0.7f,0.84f,1,1);
		
		Group.DrawTransScal(gl, mTex_BarS		,0,-.84f,1,1);
		Group.DrawTransScal(gl, mTex_freegold	,0,-.84f,mSel==31?1.1f:1,mSel==31?0.5f:1);
	...
	boolean Handle_BUY(MotionEvent event)
	{
		mSel =0;
		for(int i =0;i<7;i++){
			if(Group.CircRectsOverlap(sx + move -.7f+i* .42f+.02f,-.05f, mTex_Shop.width()*.4f, mTex_Shop.Height()*.4f, Group.screen2worldX(event.getX()), Group.screen2worldY(event.getY()),.05f))
				mSel =i+1;
		}
		
		if(Group.CircRectsOverlap(0.0f,-.84f, mTex_Bar.width()*.4f, mTex_Bar.Height()*.4f, Group.screen2worldX(event.getX()), Group.screen2worldY(event.getY()),.05f))
		{
			mSel =31;
		}
		...
		if(MotionEvent.ACTION_UP == event.getAction())
		{
			...
			if(Math.abs(sx)<.05){/*
				...
				case 3:
					...
					break;
				case 4:
					mGR.mMainActivity.onBuyHTT_GIFT_CLOCKS(null);
					break;
				case 5:
					mGR.mMainActivity.onBuyHTT_TRESURE_CLOCKS(null);
					break;
				case 6:
					mGR.mMainActivity.onBuyHTT_UNLOCK_MINI(null);
					break;
				case 7:
					mGR.mMainActivity.onBuyHTT_UNLOCK_ALL(null);
					break;
				case 31:
					M.GameScreen = M.GAMEFREE;
					break;
				case 32:
					M.GameScreen = M.GAMEMAIN;
					M.play(GameRenderer.mContext, R.drawable.splesh);
	...
	/***************************************************************/
	void Draw_FREE(GL10 gl)
	{
		Group.DrawTexture(gl, mTex_BG, 0, 0);
		for(int i =0;i<10;i++){
			Group.DrawTexture(gl, mTex_ShopSadow, sx + move -.7f+i* .42f+.02f,-.05f);
			Group.DrawTransScal(gl, mTex_Shop		, sx + move -.7f+i* .42f,0,mSel-10==i?1.1f:1,mSel-10==i?0.5f:1);
			Group.DrawTexture(gl, mTex_Free[i]		, sx + move -.7f+i* .42f,0.03f);
			Group.DrawTextureS(gl, mTex_BWatch		, sx + move -.75f+i* .42f,0.4f,.5f);
			mGR.Draw(gl, "x 1", 0,  sx + move -.65f+i* .42f,0.40f,0);
			if(i==8)
				mGR.Draw(gl, "Like",1,  sx + move -.68f+i* .42f,-.35f,1);
			else if(i==9)
				mGR.Draw(gl, "G+",1,  sx + move -.68f+i* .42f,-.35f,1);
			else
				mGR.Draw(gl, "Free",1,  sx + move -.68f+i* .42f,-.35f,1);
		}
		Group.DrawTransScal(gl, mTex_BarS	,-.7f,0.84f,1,1);
		Group.DrawTransScal(gl, mTex_Bar	,-.7f,0.84f,mSel==1?1.1f:1,mSel==1?0.5f:1);
		Group.DrawTransScal(gl, mTex_BarS	,0.7f,0.84f,1,1);
		...
		Group.DrawTransScal(gl, mTex_Back	, -.9f, -.84f,mSel==4?1.1f:1,mSel==4?0.5f:1);
		Group.DrawTexture(gl, mTex_ButtonS		, 0.9f, -.84f);
		Group.DrawTransScal(gl, mTex_Reset	, 0.9f, -.84f,mSel==5?1.1f:1,mSel==5?0.5f:1);
	}
	boolean Handle_FREE(MotionEvent event)
	{
		mSel =0;
		if(Group.CircRectsOverlap(0.0f,-.84f, mTex_Bar.width()*.4f, mTex_Bar.Height()*.4f, Group.screen2worldX(event.getX()), Group.screen2worldY(event.getY()),.05f))
		{
		...
		for(int i =0;i<10;i++){
			...
			if(Group.CircRectsOverlap(sx + move -.7f+i* .42f+.02f,-.05f, mTex_ShopSadow.width()*.4f, mTex_ShopSadow.Height()*.4f, Group.screen2worldX(event.getX()), Group.screen2worldY(event.getY()),.05f))
			{
				mSel =10+i;
			}
		}
...
//				M.GameScreen = M.GAMEBUY;
			...
			case 10:case 11:case 12:case 13:case 14:case 15:case 16:case 17:
				if(Math.abs(sx)<.05)
				{
					String s = ".*" + M.Package[mSel-10] + ".*";
					if (mGR.packages.matches(s)){
						GameRenderer.mStart.Massage("Alredy Used this watch!!!!");
					}
					else
					{
						Intent mIntent = new Intent(Intent.ACTION_VIEW);
						mIntent.setData(Uri.parse(M.LINK + M.Package[mSel-10]));
						GameRenderer.mContext.startActivity(mIntent);
					}
					System.out.print("["+mGR.packages+"] ["+M.Package[mSel-10]+"]");
				}
				break;
			case 18:
					{
				...
				case 19: 
					{
						...
						if (!mGR.packages.matches(s)) {
							...
							mGR.Clock += 1;
							Intent mIntent = new Intent(Intent.ACTION_VIEW);
							mIntent.setData(Uri.parse(M.GOOGLE));
							GameRenderer.mContext.startActivity(mIntent);
						} else
							GameRenderer.mStart.Massage("Alredy Used this watch!!!!");
					}
	...
	{
		mTex_PopUpS.drawScal(gl , 0.2f, .35f, 1.3f, .8f);
		...
		if(cout%4<2){
			int t = M.ACHIVE_VALUE[mGameSel%13][cout%4]-mGR.Total[mGameSel];
			if(t>0)
				mGR.Draw(gl, t+" left", 0,0.2f,0.15f, 1);
		}
		Group.DrawTexture(gl, mGR.mLsix.mTex_Fruit[0],cout*.28f,-.03f);
		Group.DrawTextureS(gl, mTex_NoAds  	,0.88f,.60f,.5f);
	}
...
//		if (GameRenderer.mStart.interstitialAdisReady()) 
	...
	void Draw_LevelComplete(GL10 gl)
	{
		...
		mGR.Draw(gl,  mGR.TStar+" / 65", 0, .65f, .84f, 1);
		...
		mGR.Draw(gl, "Rank: "+(int)((fillupdate/16)+1)+"-"+
				GameRenderer.mStart.getString(frog.RANK_STR[(int)(mGR.mRankFill/16)]) , 0, 0.0f,-.70f,1);
		Group.DrawTexture(gl, mTex_RankbarBG, 0.0f,-.88f);
		mTex_RankbarFil.drawSS(gl,-.32f,-.88f, Math.abs(fillupdate%16f), 1);
//		System.out.println(fillupdate+"     "+Math.abs(fillupdate%16f));
		mTex_Rankbar.drawScal(gl,-.00f,-.88f, 1.07f, 1);
...
//		Group.DrawTransScal(gl, mTex_StarShow	, 0.7f, -.84f,mSel==2?1.1f:1,mSel==2?0.5f:1);
//		Group.DrawTexture(gl, mTex_StarShow, 0.7f, -.84f);
...
//					popUp = true;
	...
	void resetLevel(int level)
	{
		...
		case M.GAMESIX:
			...
			break;
		case M.GAMESEVEN:
			mGR.mLSeven.GameResart();
			break;
		case M.GAMEEIGHT:
			mGR.mLEight.GameResart();
			break;
		case M.GAMENINE:
	...
	void ContinueLevel(int level)
	{
		...
		case M.GAMESIX:
			...
			break;
		case M.GAMESEVEN:
			mGR.mLSeven.GameContinue();
			break;
		case M.GAMEEIGHT:
			mGR.mLEight.GameContinue();
			break;
		case M.GAMENINE:
...
//					popUp = true;
	...
	void Draw_Help(GL10 gl)
	{
		...
		Group.DrawTexture(gl, mTex_LevelHelp[mGameSel], 0, 0);
		
		mGR.Draw(gl, "!! Help !!" 	, 0, 0.05f, .60f,1);
		
		Group.DrawTexture(gl, mTex_StingBS 	 ,0.00f, -.46f);
		Group.DrawTransScal(gl, mTex_StingBar,0.00f, -.46f,mSel==1?1.05f:1,mSel==1?0.5f:1);
		Group.DrawTexture(gl, mTex_PlayI  	 ,-.25f, -.46f);
		mGR.Draw(gl, "Continue" , 0		 ,0.10f, -.46f,1);
	}
	boolean Handle_Help(MotionEvent event){
		mSel = 0;
		if(Group.CircRectsOverlap(0.00f, -.46f, mTex_StingBar.width()*.4f, mTex_StingBar.Height()*.4f, Group.screen2worldX(event.getX()), Group.screen2worldY(event.getY()),.05f))
		{
			mSel =1;
		}
		if (MotionEvent.ACTION_UP == event.getAction()) {
	...
	void GTime(GL10 gl) {
		Group.DrawTransScal(gl, mGR.mTex_HelpB, 0.72f, 0.86f,mSel==1?1.05f:1,mSel==1?0.5f:1);
		Group.DrawTransScal(gl, mGR.mTex_PauseB, 0.90f, 0.86f,mSel==2?1.05f:1,mSel==2?0.5f:1);
		Group.DrawTexture(gl, mGR.mTex_Time_box, -.83f, 0.86f);
		...
		if (mGR.timesUp > 1) {
			Group.DrawTextureS(gl, mGR.mTex_Trans, 0, 0,3.2f);
			mGR.timesUp--;
			...
			if(mGR.Clock>0)
				mGR.Draw(gl, "Use Clock !", 0, -.0f, -.15f, 1);
			else
				mGR.Draw(gl, "Buy Clock !", 0, -.0f, -.15f, 1);

			Group.DrawTexture(gl, mTex_StingBS, 0,-.5f);
			Group.DrawTransScal(gl, mTex_NoThnx, 0,-.5f, mSel == 4 ? 1.05f : 1, mSel == 4 ? 0.5f : 1);
			mGR.Draw(gl, "No Thanks !", 0, 0, -.5f, 1);
...
//				System.out.println("["+(mGR.TStar+m)+" >= "+M.UNLOCK[unlockLevels]+"] [tstars = "+mGR.TStar+"] [m = "+m+"] [u = "+(unlockLevels)+"]");
	...
	void ClockScreen(MotionEvent event)
	{
		...
		if(Group.CircRectsOverlap(-.0f,-.15f, mGR.mLSelect.mTex_BarS.width()*.4f, mGR.mLSelect.mTex_BarS.Height()*.4f,
				Group.screen2worldX(event.getX()), Group.screen2worldY(event.getY()), .02f))
		{
			mSel = 3;
		}
		if(Group.CircRectsOverlap(-.0f,-.5f, mGR.mLSelect.mTex_BarS.width()*.4f, mGR.mLSelect.mTex_BarS.Height()*.4f,
				Group.screen2worldX(event.getX()), Group.screen2worldY(event.getY()), .02f))
		{
			mSel =4;
		}
...
//						M.GameScreen = M.GAMEBUY;
	...
	void Achievement(boolean fall,float dis)
	{
		...
		if(mGameSel==0||mGameSel==2||mGameSel==3||mGameSel==4||mGameSel==5||mGameSel==7||mGameSel==10){
		...
		if(mGameSel==1||mGameSel==8||mGameSel==9||mGameSel==11||mGameSel==12)
		{
...
//		}
	...
	void DrawRenkLevelUnlock(GL10 gl)
	{
		...
		Group.DrawTexture(gl, mTex_PopUpS, 0, 0);
		Group.DrawTexture(gl, mTex_PopUp , 0, 0);
		mGR.Draw(gl, "Congretulation", 0, 0, 0.30f, 1);
		mGR.Draw(gl, "New  Renk achieved!", 0, 0, 0.15f, 1);
		Group.DrawTexture(gl, mTex_BWatch	, -.14f, -.7f);
		mGR.Draw(gl, "x3  free", 0			, +.00f, -.7f, 0);
		Group.DrawTexture(gl, mTex_Rank[((int)(mGR.mRankFill/16))%10],0,-.18f);
		Group.DrawTexture(gl, mTex_RankEye[0],.03f,.07f-.18f);
		Group.DrawTextureS(gl, mTex_NoAds  	,.52f,.35f,.4f);
	}
	...
	void DrawJoin(GL10 gl)
	{
		counter++;
		Group.DrawTextureS(gl, mGR.mTex_Trans, 0, 0,3.2f);
		Group.DrawTextureS(gl, mTex_PopUpS, 0, 0,1.5f);
		Group.DrawTextureS(gl, mTex_PopUp , 0, 0,1.5f);
		mGR.Draw(gl, "Challenge your friends and", 0, 0, 0.45f, 1);
		mGR.Draw(gl, "gain achievements!"		 , 0, 0, 0.30f, 1);
		if((counter/mGR.mLOne.FrogAnim.length)%3==0)
			Group.DrawTexture(gl, mGR.mLsix.mTex_PSplash[mGR.mLOne.FrogAnim[counter%mGR.mLOne.FrogAnim.length]],0,-.1f);
		else
			Group.DrawTexture(gl, mGR.mLsix.mTex_PSplash[0],0,-.1f);
		
		Group.DrawTexture(gl, mTex_StingBS 	,0.00f, -.5f);
		Group.DrawTransScal(gl,mTex_StingBar,0.00f, -.5f,mSel==1?1.05f:1,mSel==1?0.5f:1);
		mGR.Draw(gl, "Join" 			, 0 ,0.05f, -.5f,1);
		Group.DrawTexture(gl, mTex_NoAds  	,0.75f,.50f);
	}
	boolean Handle_Join(MotionEvent event)
	{
		mSel =0;
		if(Group.CircRectsOverlap(0.00f, -.5f, mTex_StingBar.width()*.4f, mTex_StingBar.Height()*.4f, Group.screen2worldX(event.getX()), Group.screen2worldY(event.getY()),.05f))
		{
			mSel =1;
		}
