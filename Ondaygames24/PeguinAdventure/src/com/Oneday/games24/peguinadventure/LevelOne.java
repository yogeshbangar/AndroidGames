...
public class LevelOne {
	...
	GameRenderer mGR;
	public LevelOne(GameRenderer _GameRenderer)
	{
		mGR = _GameRenderer;
		...
		Bitmap b		= GameRenderer.LoadImgfromAsset("l1/color_penguin.png");
		mTex_Frog		= new SimplePlane[5][6];
		int k=0;
		for(int i=0;i<mTex_Frog.length;i++)
		{
			mTex_Frog[i]	= new SimplePlane[6];
			for(int j=0;j<mTex_Frog[i].length;j++)
			{
				mTex_Frog[i][j] = GameRenderer.addBitmap(Bitmap.createBitmap(b, (k%8)*b.getWidth()/8, (k/8)*b.getHeight()/4,b.getWidth()/8, b.getHeight()/4, null, true));
				k++;
			}
		}
