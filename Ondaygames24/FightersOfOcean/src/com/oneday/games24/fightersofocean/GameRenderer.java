	...
	public void onDrawFrame(GL10 gl) 
	{
		...
		if(mStart.adView!=null)
		{
			...
			/*	if(M.GameScreen == M.GAMELOAD)
			{
				...
				if(inv==AdView.INVISIBLE){try{handler1.sendEmptyMessage(AdView.VISIBLE);}catch(Exception e){}}
			}
			else
			{
				int inv=mStart.adViewBig .getVisibility();
				if(inv==AdView.VISIBLE){try{handler1.sendEmptyMessage(AdView.INVISIBLE);}catch(Exception e){}}
			}
