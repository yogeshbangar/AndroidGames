...
//	private final float[] mRGBA = new float[] { 1.0f, 1.0f, 1.0f, 1.0f };
	...
	public void drawBlack(GL10 gl,float x,float y,float r,float g,float b,float t,float scal){
 ...
 * gl.glScissor((int)world2screenx(-.69f), (int)world2screenY(.80f), (int)(M.ScreenWidth*.8f), (int)(M.ScreenHieght*.8f));
 * 
 * 
 * 
 * gl.glDisable(GL10.GL_SCISSOR_TEST);
	*/
	...
	public void drawRotet(GL10 gl,float _x,float _y,float r,float sx,float sy) 
	{
		if (mShouldLoadTexture) {
			loadGLTexture(gl);
			mShouldLoadTexture = false;
	...
	public void draw360(GL10 gl,float _x,float _y,float r,float dwr,float dhr) 
	{
		if (mShouldLoadTexture) {
			loadGLTexture(gl);
			mShouldLoadTexture = false;
		...
		gl.glRotatef(r, 0, 0, 1);
		gl.glTranslatef(width()*dwr,Height()*dhr,0);
	...
	public void drawRGB(GL10 gl,float x, float y, float r,float g,float b)
    {
		 if (mShouldLoadTexture) {
	            loadGLTexture(gl);
	            mShouldLoadTexture = false;
...
//		mRGBA[3] = alpha;
	...
	public void loadBitmapString(String bitmap) { // New function.
		this.str = bitmap;
		mShouldLoadTexture = true;
	}
...
//        gl.glTexEnvf(GL10.GL_TEXTURE_ENV, GL10.GL_TEXTURE_ENV_MODE, GL10.GL_REPLACE);
        ...
        if(mBitmap==null)
        {
        	mBitmap = GameRenderer.LoadImgfromAsset(this.str);
        	float sx = (mBitmap.getWidth() / M.mMaxX);
        	float sy  = (mBitmap.getHeight() / M.mMaxY);
