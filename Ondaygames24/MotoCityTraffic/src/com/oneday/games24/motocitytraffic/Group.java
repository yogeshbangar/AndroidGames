				...
				if((MotionEvent.ACTION_UP == event.getAction()) && (Counter>100)
				   && CirCir(0.9f,.9f,.11f,screen2worldX(event.getX()), screen2worldY(event.getY()), .05f))
				   {
					 ...
					 else
					 {
						...
						M.BGPlay(GameRenderer.mContext,R.drawable.bg);
					 }
				   }
				break;
			case M.GAMESPLASH:
				...
				break;
			case M.GAMEFREECOIN:
				  if(event.getAction() == MotionEvent.ACTION_UP && mScal>=1)
				  {
					  M.GameScreen = M.GAMEWORLD;
				  }
				break;
...
void DrawSplash(GL10 gl)
{
	 DrawTexture(gl,mGR.mTex_WorldBG[4],0,0);
	   ...
	   DrawTextureSS(gl,mGR.mTex_SplashFont,0-mGR.mTex_SplashFont.width()*.4f,.389f,mScal,1f,mGR.mTex_SplashFont.width()*.4f,0);
	   DrawTexture(gl,mGR.mTex_Tap2Play,0,-.3f);
}
void DrawFreeCoin(GL10 gl)
{
	DrawTexture(gl,mGR.mTex_WorldBG[4],0,0);
	DrawTextureS(gl,mGR.mTex_TransBg,0,0 ,mScal);
	DrawTextureS(gl,mGR.mTex_SmallPup,0,0,mScal);
	mGR.mFont.Draw_StringS(gl,"Congratulation",0,.43f,1,mScal);
	mGR.mFont.Draw_StringS(gl,"You have got 500 Coins",0,.1f,1,mScal);
	DrawTextureS(gl,mGR.mTex_BigBox,-.16f+.089f,-.24f,mScal);
	mGR.mFont.Draw_StringS(gl,"500",-.13f+.089f,-.24f,1,mScal);
	DrawTextureS(gl,mGR.mTex_CoinIcn,.069f+.089f,-.24f,mScal);
	
	DrawTextureS(gl,mGR.mTex_Tap2Play,0,-.519f,mScal);
	mScal*=1.6f;
   if(mScal>=1.2f)
	  mScal=1f;
...
// 	 DrawTransScal(gl,mGR.mTex_Add    ,.409f,.909f,mGR.mSel==5?1.2f:1,mGR.mSel==5?.5f:1);
 	 ...
 	 DrawTransScal(gl,mGR.mTex_CntrlBtn     ,-.799f ,-.799f,mGR.mSel==7?1.2f:1,mGR.mSel==7?.5f:1);
 	 DrawTransScal(gl,mGR.mTex_ChallangeBtn ,.799f  ,-.799f,mGR.mSel==8?1.2f:1,mGR.mSel==8?.5f:1);
	...
	for(int i=0;i<mGR.mWorld.length;i++)
	{
	 ...
	 if(mGR.mMove==0 && !mGR.mWorld[mGR.mWorldSel].isUnLock)
	  ...
	  {
		  DrawTextureS(gl,mGR.mTex_AchieUnlk,mGR.mAchieveAni.x,mGR.mAchieveAni.y,mGR.mAchieveAni.z);
		  mGR.mAchieveAni.updateAchie();
  	  }
     updateWorld();
}
float findSmall()
...
//		 mGR.mSel=5; //TotalCoin
	 ...
	 if(CircRectsOverlap(-.799f,-.799f,mGR.mTex_CntrlBtn.width()*.4f,mGR.mTex_CntrlBtn.Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
	 {
		 mGR.mSel=7; //Control
	 }
	 if(CircRectsOverlap(.799f,-.799f,mGR.mTex_ChallangeBtn.width()*.4f,mGR.mTex_ChallangeBtn.Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
	 {
...
//	 DrawTransScal(gl,mGR.mTex_Add    ,.409f,.909f,mGR.mSel==11?1.2f:1,mGR.mSel==11?.5f:1);
	 ...
	 DrawTransScal(gl,mGR.mTex_CntrlBtn  ,-.809f,-.809f,mGR.mSel==6?1.2f:1,mGR.mSel==6?.5f:1);
	 DrawTransScal(gl,mGR.mTex_AchiveBtn ,-.599f,-.809f,mGR.mSel==7?1.2f:1,mGR.mSel==7?.5f:1);
...
boolean HandleShop(MotionEvent event)
{
	mGR.mSel=0;
 	 HandleUnLock(event);
 	 HandleUpGrde(event);
	if(CircRectsOverlap(-.809f,-.809f, mGR.mTex_CntrlBtn.width()*.4f, mGR.mTex_CntrlBtn.Height()*.5f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
	{
		mGR.mSel=6;	//Control
	}
	if(CircRectsOverlap(-.599f,-.809f, mGR.mTex_AchiveBtn.width()*.4f, mGR.mTex_AchiveBtn.Height()*.5f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
	{
		mGR.mSel=7;	//Achieve
	}
...
boolean HandleCoinShop(MotionEvent event)
{
	...
	if(CircRectsOverlap(.349f,y-i*dy,mGR.mTex_BuyIcn.width()*.4f,mGR.mTex_BuyIcn.Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
	{
		mGR.mSel =i+100;
	}
	if(event.getAction() == MotionEvent.ACTION_UP)
...
//	 mGR.mFont.Draw_String(gl, mGR.mScore+"/"+(mGR.mBestStage+1)*100,.18f,.429f,0);
	 ...
	 DrawTransScal(gl,mGR.mTex_AchiveBtn ,-.809f,-.789f,mGR.mSel==4?1.2f:1,mGR.mSel==4?.5f:1);
	 ...
	 DrawTransScal(gl,mGR.mTex_ChallangeBtn ,.579f,-.789f,mGR.mSel==7?1.2f:1,mGR.mSel==7?.5f:1);
...
//		 mGR.mSel=3; //TotalCoin
	 ...
	 if(CircRectsOverlap(-.809f,-.789f,mGR.mTex_AchiveBtn.width()*.4f,mGR.mTex_AchiveBtn.Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
	 {
		 mGR.mSel=4; //Achieve
	 }
	 ...
	 if(CircRectsOverlap(.579f,-.789f,mGR.mTex_AchiveBtn.width()*.4f,mGR.mTex_AchiveBtn.Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
	 {
...
//    }
	for(int i=0;i<mGR.mCar.length;i++)
	{
	    ...
	    if(mGR.mCar[i].x !=100f && mGR.mCar[i].y!=100f)
	    {
			...
			{
		    	 M.CoinSound(GameRenderer.mContext,R.drawable.coin);
		    	 ...
		    	 else
		    	 {
		    		mGR.mTotalCoin+=2;
		    	    mGR.mLevelCoin+=2;
		    	    mGR.mCoinAni.setAchie(0,.99f,50);
		    	 }
...
//		    	 System.out.println("In Resettttttttttttt"+mGR.mCar[i].vx);
	...
	if(mGR.mTunnel.mActivetime>0 && mGR.mTunnel.x>-1 && mGR.mTunnel.x<1)
	{
	   ...
	   if(mGR.mTunnel.mActivetime<=0)
	   {
		   ...
		   for(int i=0;i<mGR.mCar.length;i++)
		   {
			  if(mGR.mCar[i].isTuunel)
				mGR.mCar[i].isTuunel=false;
		   }
	   }
	} 
	...
	for(int i=0;i<mGR.mCar.length;i++)
	{
		...
		{
  		   boolean isAcc =false; 
		   if((!mGR.mCar[i].isBridge && !mGR.mCar[j].isBridge) && (!mGR.mCar[i].isTuunel && !mGR.mCar[j].isTuunel))
...
//				      System.out.println("JX============="+mGR.mCar[j].x+"JY============== "+mGR.mCar[j].y+"i=========== "+j);
	...
	if(mGR.isCrash)
	{
	  ...
	  if(mGR.mOverCnt==1)
	  {
		  M.Crash1Sound(GameRenderer.mContext,R.drawable.carcrash0);
		  M.Crash2Sound(GameRenderer.mContext,R.drawable.carcrash1);
...
//		  mGR.mStageRem     =  mGR.mScore%100;
		  ...
		  else
		  {
			  ...
			  if(mGR.mScore>0 /*&& mGR.noStage>0*/)
			  {
			     ...
			     diff        = (33f/div);
			     diff        %=33;
//			     System.out.println("Counterrrr===Diff "+diff+"mscore===========     "+mGR.mScore+"           "+(mGR.mBestStage+1)*100+"        "+div);
			  }
			  else
			  {
				  diff=0;
//				  System.out.println("Counterrrr elseee==== "+mGR.mstageCounter+"mscore============   "+mGR.mScore);
			  }
			  mGR.mstageCounter = (int)diff;
			  
		  }
		  GameRenderer.mStart.Submitscore(R.string.leaderboard_best_stage,mGR.mBestStage);
		  GameRenderer.mStart.Submitscore(R.string.leaderboard_high_score,mGR.mBestScore);
	  }
	}
	else
	{
...
void CheckChallenge()
{
	 ...
	 {
		 if(mGR.mChallangeSec>=sec[i])
		 {
			 if(!mGR.isChalleng[i])
			 {
			    mGR.isChalleng[i]=true;
			    mGR.mTotalCoin+=M.ChallengeCoin[i];
			    mGR.mChallenge.setAchie(0,.99f,70);
...
void DrawGameplay(GL10 gl)
{
	...
	for(int i=0;i<mGR.mCar.length;i++)
	{
	  if((mGR.mBridge.x>1 && mGR.mBridge.y>1) &&(mGR.mTunnel.x>1 && mGR.mTunnel.y>1))
	  {
        ...
        {
    	   DrawBoost(i,gl);
    	  if(!mGR.mCar[i].isBridge || mGR.mCar[i].isTuunel)
	        DrawTextureRST(gl,mGR.mTex_Car[mGR.mCar[i].no],mGR.mCar[i].x,mGR.mCar[i].y,1,mGR.mCar[i].isTuunel?.5f:1f,mGR.mCar[i].ang);
        }
	  }
	}
	if(mGR.mTunnel.mActivetime>0 && mGR.mTunnel.x>-1f && mGR.mTunnel.x<1f)
	{
		...
		{
			DrawBoost(i,gl);
		    if(mGR.mCar[i].isTuunel && !mGR.mCar[i].isCollide && mGR.mCar[i].x>-1.5f && mGR.mCar[i].x<1.5f && mGR.mCar[i].y>-1.5f && mGR.mCar[i].y<1.5f)
	           DrawTextureRST(gl,mGR.mTex_Car[mGR.mCar[i].no],mGR.mCar[i].x,mGR.mCar[i].y,1,mGR.mCar[i].isTuunel?.5f:1f,mGR.mCar[i].ang);
		}	
	...
	{
	  DrawTextureS(gl,mGR.mTex_ChallengComp,mGR.mChallenge.x,mGR.mChallenge.y+.3f,mGR.mChallenge.z);
	  mGR.mChallenge.updateAchie();
	}
				...
				switch(mGR.mCar[i].mTap)
			 	{
				  case 1:
					  if(mGR.mCar[i].no!=15)
						{
						  if(M.mRand.nextBoolean())
				   	        M.CarPaas1Sound(GameRenderer.mContext,R.drawable.car_pass0);
						  else
						    M.CarPaas2Sound(GameRenderer.mContext,R.drawable.car_pass1);
					 	}
					   else
						    M.PoliceSound(GameRenderer.mContext,R.drawable.police_car);
					  ...
					  {
						mGR.mScore+=100;  
					    mGR.mScoreAni.setAchie(0,.99f,50);
					    ResetBoost(i,-100,-100);
					  }
					break;
			 	  case 2:
...
void CarAchieve()
{
	...
	for(int i=0;i<carCross.length;i++)
	{
		if(carCross[i]== mGR.mNoCrossCar)
		{
			...
			{
				mGR.isAchie[i]=true;
				mGR.mAchieveAni.setAchie(0,.99f,70);
				GameRenderer.mStart.UnlockAchievement(M.Achivemnet[i]);
			}
		}
	}
}
	...
	for(int i=1;i<mGR.mWorld.length;i++)
	{
		if(mGR.mWorld[i].isUnLock)
		{
			...
			if(!mGR.isAchie[k])
			{
				mGR.isAchie[k]=true;
				mGR.mAchieveAni.setAchie(0,.99f,70);
				GameRenderer.mStart.UnlockAchievement(M.Achivemnet[k]);
			}
		}
	}
}
...
void DrawChallenge(GL10 gl)
{
	float dy=.29f;
	  ...
	  for(int i=0;i<M.Challenge.length;i++)
	  {
	     ...
	     DrawTexture(gl ,mGR.mTex_ChallengRW[0],-.679f-.079f,ChallengY-i*dy);
	     if(mGR.isChalleng[i])
    	   DrawTexture(gl ,mGR.mTex_ChallengRW[1],-.679f-.079f,ChallengY-i*dy);
