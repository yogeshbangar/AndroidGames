...
//		Handle(event);
	...
	{
		mGR.mSel = 0;
		if(CircRectsOverlap(-.35f,-.24f, mGR.mTex_join.width()*.4f, mGR.mTex_join.Height()*.4f, screen2worldX(event.getX()), screen2worldY(event.getY()), .05f))
		{
			mGR.mSel = 1;
		}
		if(CircRectsOverlap(0.35f,-.24f, mGR.mTex_join.width()*.4f, mGR.mTex_join.Height()*.4f, screen2worldX(event.getX()), screen2worldY(event.getY()), .05f))
		{
			mGR.mSel = 2;
		}
	...
	void Draw_Control(GL10 gl){
		...
		for(int i =0;i<4;i++){
			...
			DrawTransScal(gl, mGR.mTex_DSelect	,-.43f, .51f-i*.16f,mGR.mSel-2 == i?1.2f:1,mGR.mSel-2 == i?0.5f:1);
			if(mGR.mLevel == i)
		...
		for(int i =0;i<5;i++)
			DrawTransScal(gl, mGR.mTex_Icn[i],-.67f+i*.35f, -.68f,mGR.mSel-10 == i?1.1f:1,mGR.mSel-10 == i?0.5f:1);
		
		DrawTransScal(gl, mGR.Ads_Free	, .73f, -.14f,mGR.mSel == 9?1.1f:1,mGR.mSel == 9?0.5f:1);
	...
	boolean Handle_Control(MotionEvent event)
	{
		...
		if(CircRectsOverlap(.73f, -.14f, mGR.Ads_Free.width()*.4f, mGR.Ads_Free.Height()*.4f, screen2worldX(event.getX()), screen2worldY(event.getY()), .05f))
		{
			mGR.mSel = 9;
		}
		for(int i =0;i<5;i++){
			if(CircRectsOverlap(-.67f+i*.35f,-.68f, mGR.mTex_Icn[0].width()*.4f, mGR.mTex_Icn[0].Height()*.4f, screen2worldX(event.getX()), screen2worldY(event.getY()), .05f))
			{
				mGR.mSel = i+10;
			}
		}
	...
	void Draw_Help(GL10 gl){
		...
		DrawTransScal(gl, mGR.mTex_Back	, -.68f, -.54f,mGR.mSel == 1?1.1f:1,mGR.mSel == 1?0.5f:1);
	}
...
//		M.sound3(GameRenderer.mContext, R.drawable.cube1);
	...
	void Gamelogic()
	{
		...
		if(mGR.moveDirection == 2 && GameCount <3)
		{
			for (int i = 0; i < mGR.mNum.length; i++) {
		...
		if(mGR.moveDirection == 3 && GameCount <3)
		{
		...
		if(mGR.moveDirection == 4 && GameCount <3)
		{
...
//								M.sound2(GameRenderer.mContext, R.drawable.cube_apear);
			...
			if((mGR.max == 512 && mGR.mLevel == 0)||
					(mGR.max == 1024 && mGR.mLevel == 1)||
					(mGR.max == 2048 && mGR.mLevel == 2))
			{
...
//				try{handler.sendEmptyMessage(0);}catch(Exception e){}
	...
	void Draw_GamePlay(GL10 gl)
	{
		...
		if(mGR.mLevel < 3)
			DrawTexture(gl, mGR.mTex_Top_pattern, 0, .61f+goy);
		if(mGR.mLevel == 0){//0
			DrawTexture(gl,mGR.mTex_ScorePreview, -.61f+getPower(mGR.max)*2.04f*.066f, .67f+goy);
			drawNumberScal(gl, mGR.max, -.69f+getPower(mGR.max)*2.04f*.066f, .67f+goy, .6f, 1);
			DrawTexture(gl, mGR.mTex_Mark, -.61f+getPower(mGR.max)*2.04f*.066f, .62f+goy);
			mGR.mTex_Fill.drawSS(gl, -.61f,.59f+goy, getPower(mGR.max)*2.04f, 1);//10
			
		}
		if(mGR.mLevel == 1){
			DrawTexture(gl,mGR.mTex_ScorePreview, -.61f+getPower(mGR.max)*1.842f*.066f, .67f+goy);
			drawNumberScal(gl, mGR.max, -.69f+getPower(mGR.max)*1.842f*.066f, .67f+goy, .6f, 1);
			DrawTexture(gl, mGR.mTex_Mark, -.61f+getPower(mGR.max)*1.842f*.066f, .62f+goy);//.06f	
			mGR.mTex_Fill.drawSS(gl, -.61f,.59f+goy, getPower(mGR.max)*1.842f, 1);//10
		}
		if(mGR.mLevel == 2){
			DrawTexture(gl,mGR.mTex_ScorePreview, -.61f+getPower(mGR.max)*1.675f*.066f, .67f+goy);
			drawNumberScal(gl, mGR.max, -.69f+getPower(mGR.max)*1.675f*.066f, .67f+goy, .6f, 1);
			DrawTexture(gl, mGR.mTex_Mark, -.61f+getPower(mGR.max)*1.675f*.066f, .62f+goy);
			mGR.mTex_Fill.drawSS(gl, -.61f,.59f+goy, getPower(mGR.max)*1.675f, 1);//10
		}
		
//		DrawTexture(gl, mGR.mTex_Bottum		, 0, -.51f);
		DrawTexture(gl, mGR.mTex_GameBoard, 0, .05f+goy);
		
		
		if(M.GameScreen == M.GAMEPLAY){
			DrawTransScal(gl, mGR.mTex_MenuB		, -.6f,.60f,mGR.mSel == 1?1.1f:1,mGR.mSel == 1?0.5f:1);
			DrawTransScal(gl, mGR.mTex_RestartBtn	,  .6f,.60f,mGR.mSel == 2?1.1f:1,mGR.mSel == 2?0.5f:1);
		}
		...
		for(int i =0;i<mGR.mNum.length;i++)
		{
			for(int j =0;j<mGR.mNum[i].length;j++)
			{
//				DrawTexture(gl, mGR.mTex_Block[0],-.57f+.38f*j,0.39f-.23f*i);
				if(mGR.mNum[i][j].num > 0){
					if(i==si && j == sj && scl <1)
						DrawTextureS(gl, mGR.mTex_Block[getPower(mGR.mNum[i][j].num)],-.57f+.38f*j,+goy+0.39f-.23f*i,scl);
					else
						DrawTexture(gl, mGR.mTex_Block[getPower(mGR.mNum[i][j].num)],-.57f+.38f*j,+goy+0.39f-.23f*i);
				}
			}
		}
		if(mGR.mHelp ==0){
		...
		if(M.GameScreen == M.GAMEOVER || M.GameScreen == M.GAMEWIN)
		{
			...
			for(int i=0;i<3&&i<=scot;i++){
				if(i==scot)
					DrawTextureS(gl, mGR.mTex_Star0, -.43f+.43f*i, -.05f,SScal);
				else
					DrawTexture(gl, mGR.mTex_Star0, -.43f+.43f*i, -.05f);
			}
			if(SScal<=1){
		...
		else if(M.GameScreen == M.GAMEJOIN)
		{
			DrawFlip(gl, mGR.mTex_Bg, 0, 0);
			DrawTexture(gl, mGR.mTex_Small_popup, 0, 0);
			DrawTexture(gl, mGR.mTex_chalg, 0, -.04f);
			DrawTransScal(gl, mGR.mTex_join	,-.35f,-.24f,mGR.mSel == 1?1.1f:1,mGR.mSel == 1?0.5f:1);
			DrawTransScal(gl, mGR.mTex_Later	,0.35f,-.24f,mGR.mSel == 2?1.1f:1,mGR.mSel == 2?0.5f:1);
		}
		else
			Gamelogic();
	}
...
//		if(mGR.moveDirection == 4 && GameCount <3)
	...
	void MultiPlay()
	{
		Intent mIntent = new Intent(GameRenderer.mContext,SkeletonActivity.class);
