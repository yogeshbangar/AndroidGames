...
//	private final float[] mRGBA = new float[] { 1.0f, 1.0f, 1.0f, 1.0f };
	...
	public void drawRGBTS(GL10 gl,float x,float y,float r, float g,float b,float t,float sx,float sy) 
	{
		gl.glPushMatrix();
		gl.glFrontFace(GL10.GL_CCW);
		gl.glEnable(GL10.GL_CULL_FACE);
	...
	public void drawRTS(GL10 gl,float _x,float _y,float z,float ang,float posx,float posy) 
	{
		gl.glPushMatrix();
		gl.glFrontFace(GL10.GL_CCW);
		gl.glEnable(GL10.GL_CULL_FACE);
		...
		gl.glScalef((M.ScreenHieght/M.ScreenWidth)*z,z,0);
		gl.glRotatef(ang, 0, 0, 1);
		gl.glTranslatef(width()*posx,Height()*posy,0);
	...
	public void drawDarkS(GL10 gl,float x, float y, float zx, float zy,float t)
    {
        gl.glPushMatrix();
        gl.glEnableClientState(GL10.GL_VERTEX_ARRAY);
        gl.glVertexPointer(3, GL10.GL_FLOAT, 0, mVerticesBuffer);
...
//		mRGBA[3] = alpha;
	...
	public Bitmap getBitmap() { // New function.
		return this.mBitmap;
	}
