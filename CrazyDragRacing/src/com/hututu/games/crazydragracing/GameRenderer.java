...
public class GameRenderer implements Renderer 
{
	...
	SimplePlane   mTex_Pointer, mTex_Hightbar,mTex_Skip,mTex_Leader,mTex_Achiv,mTex_ShopBox,mTex_$; //AdHouse
...
//	private Handler handler = new Handler() {public void handleMessage(Message msg) {mStart.adView.setVisibility(msg.what);}};
	...
	void init()
	{
		...
		try
		{
			...
			mTex_$				= add("doller.png");
			mTex_ShopBox		= add("shop_box.png");
			mTex_Leader			= add("leader.png");
			mTex_Achiv			= add("achieve.png");
			...
			mTex_About[1]		= add("ui/Face_Changer.png");
			
			mTex_Help			= new SimplePlane[2];
			mTex_Help[0]		= add("ui/help.png");
			mTex_Help[1]		= add("ui/Crazy_Boat Racing.png");
...
//		unlockLevel = 6;
		...
		if(breakPoint<2)
		{
			mCar[1].set(-.5f,-.15f,breakPoint%mTex_Car.length,1+(breakPoint*4+mLevel[breakPoint].mUPower)/19f,0);
...
//		System.out.println("mLvlLength = "+mLvlLength);
		...
		adsCount++;	
		if(adsCount%2==0){
			try {handler5.sendEmptyMessage(0);}catch(Exception e){}
		}
//		for(int i=0;i<mLevel.length;i++)
//			mLevel[breakPoint].mULevel = 4;
//		unlockLevel =6;
	}
	int adsCount = 0;
	private Handler handler5 = new Handler() {public void handleMessage(Message msg) {GameRenderer.mStart.load();}};//AdHouse
