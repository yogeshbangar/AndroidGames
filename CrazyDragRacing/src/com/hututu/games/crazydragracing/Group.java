...
//		Handle(event);
//		System.out.println(mGR.breakPoint);
...
//	
	void DrawWinOver(GL10 gl)
	{
		...
		if(M.GameScreen == M.GAMEWIN)
		{
			...
			mGR.mFont.draw(gl, M.MORE, +.70f,-.05f, 2, 0);
		}
		else
		{
			...
			mGR.mFont.draw(gl, M.HINT, +.70f,-.05f, 2, 0);
		}
		mGR.mFont.draw(gl, M.MENU[1], -.83f,-.05f, 2, 0);
	...
	boolean HandleWinOver(MotionEvent event)
	{
		...
		if((Counter>60) && CircRectsOverlap(-.829f,-0.839f,mGR.mTex_MenuBut[0].width()*.4f,mGR.mTex_MenuBut[0].Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
		{
				mGR.mSel = 14;//NEXT
		}
...
//				MoreGame();
	...
	void Draw_Level(GL10 gl)
	{
		...
		mGR.mFont.draw(gl, M.MENU[1], -.83f,-.05f, 2, 0);
		mGR.mFont.draw(gl, M.MORE, 0.65f,-.05f, 2, 0);
		...
		if(M.GAMEJOIN == M.GameScreen)
		{
			mGR.mTex_SetBG.drawTransprent(gl, 0, 0, .7f);
			mGR.mTex_Road.drawRS(gl	,.6f,1,180, 0,0.00f);
			mGR.mFont.draw(gl, "Challenge your friends"	, 0,0.23f, 2, 1);
			mGR.mFont.draw(gl, "and gain achievemets"	, 0,0.08f, 2, 1);
			DrawTexture(gl, mGR.mTex_MenuBut[0], -.30f,-.30f);
			mGR.mFont.draw(gl, "Join us"	, -.30f,-.30f, 1, 1);
			if(mGR.mSel == 97)
				DrawTexture(gl, mGR.mTex_MenuBut[1], -.30f,-.30f);
			DrawTexture(gl, mGR.mTex_MenuBut[0], 0.30f,-.30f);
			mGR.mFont.draw(gl, "Later"	, 0.30f,-.30f, 1, 1);
			if(mGR.mSel == 98)
				DrawTexture(gl, mGR.mTex_MenuBut[1], 0.30f,-.30f);
...
//					System.out.println(M.PHSCROSS);
	...
	public boolean Handle_Join(MotionEvent event)
	{
		mGR.mSel = 0;
		if(CircRectsOverlap(-.30f,-.30f,mGR.mTex_MenuBut[0].width()*.4f,mGR.mTex_MenuBut[0].Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
		{
				mGR.mSel = 97;//Join
		}
		if(CircRectsOverlap(0.30f,-.30f,mGR.mTex_MenuBut[0].width()*.4f,mGR.mTex_MenuBut[0].Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
		{
				mGR.mSel = 98;//Later
		}
	...
	public boolean Handle_Buy(MotionEvent event)
	{
		mGR.mSel = 0;
		for(int i=0;i<3;i++){
			if(CircRectsOverlap(0.373f,0.305f-.355f*i,mGR.mTex_LevelBut[0].width()*.4f,mGR.mTex_LevelBut[0].Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
			{
					mGR.mSel = i+1;//Back
			}
		}
		if(CircRectsOverlap(.38f,-.76f,mGR.mTex_MenuBut[0].width()*.4f,mGR.mTex_MenuBut[0].Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
		{
				mGR.mSel = 4;//Later
		}
		...
		if(MotionEvent.ACTION_UP == event.getAction())
		{
			...
			case 1:
				M.GameScreen = M.GAMESHOP;
				 mGR.mMainActivity.onBuyCoin10000(null);
				break;
			case 2:
				M.GameScreen = M.GAMESHOP;
				 mGR.mMainActivity.onBuyCoin30000(null);
				break;
			case 3:
				mGR.mMainActivity.onBuyCoin150000(null);
				M.GameScreen = M.GAMESHOP;
				break;
			case 4:
				M.GameScreen = M.GAMESHOP;
	...
	void Draw_Shop(GL10 gl)
	{
		...
		mGR.mFont.draw(gl, M.BUYMORE,+.62f,-.05f, 2, 0);
		if(mGR.mSel == 16)
			DrawTexture (gl, mGR.mTex_LevelBut[0],+.775f,-0.052f);
		...
		if (M.GameScreen == M.GAMEBUY) {
			mGR.mTex_SetBG.drawTransprent(gl, 0, 0, .7f);
			for(int i=0;i<3;i++){
				DrawTexture(gl, mGR.mTex_ShopBox, 0, .3f-.355f*i);
				mGR.mFont.draw(gl, "BUY", .38f, .3f-.355f*i, 2, 1);
				mGR.mFont.draw(gl, strcoin[i], -.23f, .35f-.355f*i, 1, 1);
				DrawTexture(gl, mGR.mTex_$, -.08f, .35f-.355f*i);
				if(mGR.mSel == i+1)
					DrawTexture(gl, mGR.mTex_LevelBut[0],0.373f,0.305f-.355f*i);
			}
			DrawTexture(gl, mGR.mTex_MenuBut[0], .38f,-.76f);
			mGR.mFont.draw(gl, "LATER", .38f,-.76f, 2, 1);
			if(mGR.mSel == 4)
				DrawTexture(gl, mGR.mTex_MenuBut[1], .38f,-.76f);
		}
	}
	final String strcoin[] ={"20000 Coins   0^99", "50000 Coins   1^99", "150000 Coins  4^99"};
	...
	void Upgrade()
	{
		...
		{
			//M.GameScreen = M.GAMEBUY;
			GameRenderer.mStart.Dont(M.DONTCOIN);
		}
	}
	/******************************************************************************
	 * @param Shop END
	...
	void Draw_HELP_ABOUT(GL10 gl)
	{
		...
		if(M.GameScreen == M.GAMEHELP){
			...
			DrawTexture (gl, mGR.mTex_Help[1],0.00f,0.55f);
			DrawTexture (gl, mGR.mTex_Help[0],-.01f,-.566f);
		}
		else{
			mGR.mFont.draw(gl, "ABOUT US", -.18f, .7f, 2, 0);
			DrawTexture (gl, mGR.mTex_About[1],0,0.55f);
...
//			System.out.println("-----------------------------------------------------------------");
	...
	void Draw_Manu(GL10 gl)
	{
		...
		DrawTransScal(gl, mGR.mTex_Achiv, 0.59f,.76f,mGR.mSel==6?1.1f:1,mGR.mSel==6?0.5f:1);
		DrawTransScal(gl, mGR.mTex_Leader,0.82f,.76f,mGR.mSel==7?1.1f:1,mGR.mSel==7?0.5f:1);
	...
	public boolean Handle_Manu(MotionEvent event)
	{
		...
		if(CircRectsOverlap(0.59f,.76f,mGR.mTex_MenuBut[0].width()*.4f,mGR.mTex_MenuBut[0].Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
		{
			mGR.mSel = 6;
		}
		if(CircRectsOverlap(0.82f,.76f,mGR.mTex_MenuBut[0].width()*.4f,mGR.mTex_MenuBut[0].Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
		{
			mGR.mSel = 7;
		}
		if(MotionEvent.ACTION_UP == event.getAction())
...
//				System.out.println("0~~~~~[SLeve = "+mGR.mLevel[mGR.breakPoint].mSLevel+"] [ULeve = "+mGR.mLevel[mGR.breakPoint].mULevel+"] [BreakPoint = "+mGR.breakPoint+"] [unlockLevel = "+mGR.unlockLevel+"]");
				...
				else if(mGR.breakPoint == (mGR.unlockLevel-1) && mGR.unlockLevel < 7)
				{
					GameRenderer.mStart.UnlockAchievement(Achiv[mGR.unlockLevel-1]);
					mGR.unlockLevel++;
					mGR.breakPoint ++;
					mGR.breakPoint=mGR.breakPoint%mGR.mLevel.length;
					mGR.mLevel[mGR.breakPoint].mSLevel =0;
					System.out.println(mGR.unlockLevel+" ~~~~~~~~~~~~~~~~~~~~~~");
				}
				else if(mGR.breakPoint < 7)
				{
					mGR.breakPoint ++;
					mGR.breakPoint=mGR.breakPoint%mGR.mLevel.length;
					mGR.mLevel[mGR.breakPoint].mSLevel =0;
...
//		M.play(GameRenderer.mContext, R.drawable.background);
	...
	final int Achiv[]={ 
			R.string.achievement_phase_one,
			R.string.achievement_phase_two,
			R.string.achievement_phase_three,
			R.string.achievement_phase_four,
			R.string.achievement_phase_five,
			R.string.achievement_phase_six,
	};
