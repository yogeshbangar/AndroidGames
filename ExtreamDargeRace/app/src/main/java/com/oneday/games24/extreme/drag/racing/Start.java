...
public class Start extends BaseGameActivity{
	...
	public void onCreate(Bundle savedInstanceState) 
	{
		...
		Log.d("StartActivity","mGR.GameRenderer ~~~ "+mGR.gameUnlock);
	...
	void pause() {
		...
		editor.putBoolean("mIsQuckRace", mGR.mIsQuickRace);
		editor.putBoolean("citiUnlock", mGR.citiUnlock);
		editor.putBoolean("SingUpadate", mGR.SingUpdate);
		...
		editor.putInt("mDollar", mGR.mDollar);
		...
		editor.putFloat("gamedistace", mGR.gameDistance);
	...
	void resume() {
		...
		Log.d("StartActivity","mGR.gameUnlock   "+mGR.gameUnlock);
		for(int i=0;i<mGR.gameUnlock.length;i++)
			for(int j=0;j<mGR.gameUnlock[i].length;j++)
		...
		mGR.mIsQuickRace = prefs.getBoolean("mIsQuckRace", mGR.mIsQuickRace);
		
		mGR.citiUnlock = prefs.getBoolean("citiUnlock", mGR.citiUnlock);
		mGR.SingUpdate = prefs.getBoolean("SingUpadate", mGR.SingUpdate);
		...
		mGR.mDollar = prefs.getInt("mDollar", mGR.mDollar);
		...
		mGR.gameDistance = prefs.getFloat("gamedistace", mGR.gameDistance);
