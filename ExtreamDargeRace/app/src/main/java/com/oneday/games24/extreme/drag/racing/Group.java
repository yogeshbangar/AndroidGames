...
//		M.GameScreen = M.GAMEMENU;
		...
		case M.GAMELOGO:
			...
			if(Counter>60){
				...
				M.play(GameRenderer.mContext, R.raw.theme);
			}
			break;
		case M.GAMEADD:
			if(Counter>98){
				DrawTexture(gl, mGR.mTex_Cont, 0.8f, -.88f);
//				M.GameScreen = M.GAMEMENU;
			}else{
...
//		setting();
	...
	public boolean TouchEvent(MotionEvent event) {
		...
		case M.GAMEADD:
			...
			CircRectsOverlap(0.8f, -.88f, mGR.mTex_Cont.width()*.4f, mGR.mTex_Cont.Height()*.4f,
		...
		case M.GAMESPLASH:
			if(MotionEvent.ACTION_UP == event.getAction() && (ey<.8)){
				...
				Counter =0;
				M.sound12(GameRenderer.mContext, R.raw.click);
				M.GameScreen = M.GAMEMENU;
			}
			break;
		case M.GAMEWIN:
	...
	void Draw_GameOver(GL10 gl){
		...
		else if(!mGR.mIsQuickRace && M.GameScreen == M.GAMEWIN){
	...
	void Draw_Splash(GL10 gl) {
		...
		mGR.mFont.Draw_String(gl, "TAP TO CONTINUE...", .41f, -.71f, 1, 1);
		mGR.mFont.Draw_String(gl, "TAP TO CONTINUE...", .4f, -.7f, 0, 1);
	}
...
//		mGR.mFont.Draw_String(gl,"150000", 0.26f, -.49f, 0, 0);
		mGR.mFont.Draw_String(gl, mGR.mDollar +"$", 0.00f, 0.40f, 0, 0);
	}
	boolean Handle_Purchase(MotionEvent event)
	{
		...
		if(CircRectsOverlap(  0.2f, -.60f, mGR.mTex_AdsFree.width()*.7f, mGR.mTex_AdsFree.Height()*.7f, screen2worldX(event.getX()), screen2worldY(event.getY()), .01f))
		{
			mGR.mSel = 26;
		}
		if (MotionEvent.ACTION_UP == event.getAction()) {
...
//				mGR.mMainActivity.onBuyGold100000(null);//$4
				break;
			case 26:
				Log.d("RewardAds","Clicked");
				mGR.mStart.mGameAd.ShowRewardVideo();
				break;
			}
			if(mGR.mSel!=0)
				M.sound12(GameRenderer.mContext, R.raw.click);
	...
	void Draw_UPgrade(GL10 gl) {
		DrawTexture(gl, mGR.mTex_CommandBG, 0, 0);
...
//			DrawTexture(gl, mGR.mTex_Engin[mGR.mSpec[mGR.mBike].lvlWeight>2?1:0], -.62f + .42f * 3, .36f);
			...
			break;
		case 1:
			DrawTexture(gl, mGR.mTex_Exhaust[1], -.62f + .42f * 0, .33f);
			if(mGR.mSpec[mGR.mBike].lvlExhaust >=0){
				DrawTexture(gl, mGR.mTex_Exhaust[mGR.mSpec[mGR.mBike].lvlExhaust>0?1:0], -.62f + .42f * 1, .33f);
			}
			if(mGR.mSpec[mGR.mBike].lvlExhaust >=1){
				DrawTexture(gl, mGR.mTex_Exhaust[mGR.mSpec[mGR.mBike].lvlExhaust>1?1:0], -.62f + .42f * 2, .30f);
				DrawTexture(gl, mGR.mTex_Exhaust[mGR.mSpec[mGR.mBike].lvlExhaust>1?1:0], -.62f + .42f * 2, .42f);
			}
			if(mGR.mSpec[mGR.mBike].lvlExhaust >=2){
				DrawTexture(gl, mGR.mTex_Exhaust[mGR.mSpec[mGR.mBike].lvlExhaust>2?1:0], -.62f + .42f * 3, .30f);
				DrawTexture(gl, mGR.mTex_Exhaust[mGR.mSpec[mGR.mBike].lvlExhaust>2?1:0], -.62f + .42f * 3, .37f);
				DrawTexture(gl, mGR.mTex_Exhaust[mGR.mSpec[mGR.mBike].lvlExhaust>2?1:0], -.62f + .42f * 3, .44f);
			}
			for (int i = 0; i < 4; i++) {
...
//					mGR.mFont.Draw_String(gl, ""+(i*(GBox*(mGR.mBike+1)*Mul)), -.60f + .42f * i, .18f, 1, 1);
		...
		mGR.mFont.Draw_String(gl, mGR.mDollar +"", -.80f, -.47f, 0, 0);
		DrawTransScal(gl, mGR.fromLevel?mGR.mTex_Cont :mGR.mTex_BackB, .83f,-.89f, mGR.mSel == 20 ? 1.1f : 1, mGR.mSel == 20 ? 0.5f : 1);
			...
			case 2:case 3:case 4:
				...
				case 0:
					if(mGR.mSpec[mGR.mBike].lvlWeight>(mGR.mSel-2)){
						...
						if(mGR.mDollar >=(mGR.mSel-1)*(wieght*(mGR.mBike+1)*Mul)){
							if(mGR.mSpec[mGR.mBike].lvlWeight+1 == mGR.mSel - 1){
								mGR.mSpec[mGR.mBike].lvlWeight = mGR.mSel - 1;
								mGR.mDollar -=(mGR.mSel-1)*(wieght*(mGR.mBike+1)*Mul);
							}
						}else{
							popup = 2;
						}
				...
				case 1:
					if(mGR.mSpec[mGR.mBike].lvlExhaust>(mGR.mSel-2)){
						...
						if(mGR.mDollar >=(mGR.mSel-1)*(Exhaust*(mGR.mBike+1)*Mul)){
							if(mGR.mSpec[mGR.mBike].lvlExhaust+1 == mGR.mSel - 1){
								mGR.mSpec[mGR.mBike].lvlExhaust = mGR.mSel - 1;
								mGR.mDollar -=(mGR.mSel-1)*(Exhaust*(mGR.mBike+1)*Mul);
							}
							//mGR.mSpec[mGR.mBike].lvlExhaust = mGR.mSel - 1;
						}else{
							popup = 2;
				...
				case 2:
					if(mGR.mSpec[mGR.mBike].lvlNitro>(mGR.mSel-2)){
						...
						if(mGR.mDollar >=(mGR.mSel-1)*(Nirto*(mGR.mBike+1)*Mul)){
							if(mGR.mSpec[mGR.mBike].lvlNitro+1 == mGR.mSel - 1){
								mGR.mSpec[mGR.mBike].lvlNitro = mGR.mSel - 1;
								mGR.mDollar -=(mGR.mSel-1)*(Nirto*(mGR.mBike+1)*Mul);
							}
//							mGR.mSpec[mGR.mBike].lvlNitro = mGR.mSel - 1;
						}else{
							popup = 2;
				...
				case 3:
					if(mGR.mSpec[mGR.mBike].lvlGearBox>(mGR.mSel-2)){
						...
						if(mGR.mDollar >=(mGR.mSel-1)*(GBox*(mGR.mBike+1)*Mul)){
							if(mGR.mSpec[mGR.mBike].lvlGearBox+1 == mGR.mSel - 1){
								mGR.mSpec[mGR.mBike].lvlGearBox = mGR.mSel - 1;
								mGR.mDollar -=(mGR.mSel-1)*(GBox*(mGR.mBike+1)*Mul);
							}
//							mGR.mSpec[mGR.mBike].lvlGearBox = mGR.mSel - 1;
						}else{
							popup = 2;
...
//		mGR.mFont.Draw_String(gl, "900 METER",-.7f,-.25f-.12f*1, 1, 0);
		...
		mGR.mFont.Draw_Number(gl, mGR.mDollar,0,-.91f, 1);
...
//		mGR.mFont.Draw_String(gl,  (int)(mGR.mSpec[mGR.mBike].Nitro)+"sec"		, 0.35f, -.28f - .14f * 2, 0, 0);
		...
		if (M.GameScreen == M.GAMEMENU) {
			...
			DrawTransScal(gl, mGR.mTex_Cont, 0.80f	, -.20f, mGR.mSel == 3 ? 1.1f : 1, mGR.mSel == 3 ? 0.5f : 1);
			DrawTransScal(gl, mGR.mTex_ShopB, 0.80f	, -.50f, mGR.mSel == 4 ? 1.1f : 1, mGR.mSel == 4 ? 0.5f : 1);
			DrawTransScal(gl,mGR.mTex_GarageB, 0.80f, -.80f, mGR.mSel == 5 ? 1.1f : 1, mGR.mSel == 5 ? 0.5f : 1);
		} 
		...
		if (MotionEvent.ACTION_UP == event.getAction()) {
			...
			case 3:
				...
				if (M.GameScreen == M.GAMEPUASE) {
					...
					mGR.gameReset();
					load();
					mGR.mIsQuickRace = true;
				} else if (M.GameScreen == M.GAMESHOP) {
					if (!mGR.mBikeUnlock[mGR.mBike]) {
						if (getCost(mGR.mBike) <= mGR.mDollar) {
							mGR.mBikeUnlock[mGR.mBike] = true;
							mGR.mDollar -= getCost(mGR.mBike);
							unclockAchive();
						} else {
							popup = 2;
						}
...
//				GameRenderer.mStart.beginUserInitiatedSignIn();
	...
	void gameWinOver() {
		M.stop(GameRenderer.mContext);
		mGR.fromLevel = !mGR.mIsQuickRace;
		mGR.mPlayer.price = 0;
		mGR.mPlayer.bonus = 0;
		if (mGR.mIsGameWin == 1) {
		...
		if(!mGR.mIsQuickRace && M.GameScreen == M.GAMEWIN)
		{
...
//			}
		...
		mGR.mPlayer.Dis += mGR.mPlayer.Engine * (1.88f/mGR.gameDistance);
		mGR.mOpponent.Dis += mGR.mOpponent.Engine * (1.88f/mGR.gameDistance);
...
//					mGR.mPlayer.Engine = .005f;
	...
	void Draw_Play(GL10 gl) {
		...
		if (mGR.start > 1 && mGR.start < 5) {
			if (Counter % 15 == 0){
				mGR.start++;
				M.sound14(GameRenderer.mContext, R.raw.beep);
			}
		}
		if (mGR.start >= 5) {
			gamelogic();
	...
	public boolean Handle_Play(MotionEvent event) {
		...
		if(MotionEvent.ACTION_UP == event.getAction()){
			...
			case 3:
				...
				if (mGR.mPlayer.Gear > 2 && mGR.mPlayer.rpm < 170){
					mGR.mPlayer.Gear--;
					M.sound13(GameRenderer.mContext, R.raw.gearsift);
				}
				break;
			case 4:
