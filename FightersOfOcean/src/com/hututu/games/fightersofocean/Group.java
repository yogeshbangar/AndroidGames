...
//		Handle(event);
	...
	void Draw_Menu(GL10 gl)
	{
		...
		DrawTransScal(gl, mGR.mTex_Help	 	,-.83f,-.85f,1,mGR.mSel==4?.5f:1);
		DrawTransScal(gl, mGR.mTex_Continue	,0.00f,(mGR.resumeCounter > 60)?-.46f:-.85f,1,mGR.mSel==7?.5f:1);
		DrawTransScal(gl, mGR.mTex_Exit    	,0.83f,-.85f,1,mGR.mSel==5?.5f:1);
	...
	{
		mGR.mSel = 0;
		if(CircRectsOverlap(-.83f,-.85f,mGR.mTex_Help.width()*.4f,mGR.mTex_Help.Height()*.4f,screen2worldX(event.getX()),screen2worldY(event.getY()),.05f))
		{
			mGR.mSel = 4;//Help
		}
