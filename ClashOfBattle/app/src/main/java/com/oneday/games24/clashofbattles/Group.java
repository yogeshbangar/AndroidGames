...
//				share2friend();
			...
			case 8:
				...
				M.play(R.raw.gameplay_theme);
				fade = 1;
				break;
			}
			M.sound10(R.raw.click);
			mSel =0;
		}
		return true;
	}
	...
	void Draw_Buy(GL10 gl){
		...
		DrawTransScal(gl, mGR.mTex_RewardBtn, 0, 0,mSel==(1)?1.1f:1,mSel==(1)?0.5f:1);
		DrawTransScal(gl, mGR.mTex_Icon[11], -.88f,-.86f,mSel==16?1.1f:1,mSel==16?0.5f:1);// Back
		if(setReword)
			DrawTransScal(gl, mGR.mTex_Reward, 0, -0.7f,(Counter % 30)*.03f+ 1,1);
	}
	boolean setReword = false;
	boolean Handle_Buy(MotionEvent event){
		mSel = 0;
		if(CircRectsOverlap(.0f, .0f,  mGR.mTex_Icon[11].width()*.5f,  mGR.mTex_Icon[11].Height()*2.3f, screen2worldX(event.getX()),screen2worldY(event.getY()), .05f)){
			mSel = 1;
		}
		...
		Log.d("GameAd", "Ad was dismissed."+setReword);
		if(MotionEvent.ACTION_UP == event.getAction()&&mSel > 0){
			switch (mSel) {
			case 1:
				setReword = false;
				GameRenderer.mStart.mGameAd.showReward();
...
//		DrawTransScal(gl, mGR.mTex_Optin, .68f, -.82f, mSel == 4 ? 1.1f : 1, mSel == 4 ? 0.5f : 1);
		DrawTransScal(gl, mGR.mTex_Icon[M.setValue?1:0], 0.60f,-.82f,mSel==5?1.1f:1,mSel==5?0.5f:1);// Sound
		DrawTransScal(gl, mGR.mTex_Icon[M.setMusic?5:4], 0.85f,-.82f,mSel==6?1.1f:1,mSel==6?0.5f:1);// Music
//
	}
	boolean Handle_Menu(MotionEvent event){
		...
		if(CircRectsOverlap(0.60f,-.82f,  mGR.mTex_Icon[11].width()*.5f,  mGR.mTex_Icon[11].Height()*.253f, screen2worldX(event.getX()),screen2worldY(event.getY()), .05f)){
			mSel = 5;
		}
		if(CircRectsOverlap(0.85f,-.82f,  mGR.mTex_Icon[11].width()*.5f,  mGR.mTex_Icon[11].Height()*.253f, screen2worldX(event.getX()),screen2worldY(event.getY()), .05f)){
			mSel = 6;
		}
		if(MotionEvent.ACTION_UP == event.getAction()){
			switch (mSel) {
					...
					case 0:
						if (mGR.mPlayCar[i].NoBomb > 0 && mGR.mPlayCar[i].img != 3) {
							if(mGR.mPlayCar[i].count>300){
								...
								M.sound12(R.raw.multipal_missile);
							}
						}
						break;
					case 1:
