	...
	public void draw(GL10 gl) 
	{
		...
		case M.GameLogo:
			...
			if(counter>60)
			{
				...
				M.sound9(GameRenderer.mContext, R.raw.bike);
				counter =0;
			}
			break;
		case M.GameADD:
...
//		case M.GameSetting:
		...
		case M.GameHelp:
		case M.GameAbout:
			if(event.getAction()==MotionEvent.ACTION_UP && screen2worldY(event.getY()) <.8 ){
				if(M.setBG)
					M.Splashplay(GameRenderer.mContext,R.raw.splash);
				else
					M.loopSoundStop();
				mGR.mSel = 0;
...
//			HandleSing(event);
		case M.GameChallenge:
			if(event.getAction()==MotionEvent.ACTION_UP)
			{
				...
				{
					if(mGR.mCarSel == 0)
						M.BGplay(GameRenderer.mContext,R.raw.gameplay_bike);
					else
						M.BGplay(GameRenderer.mContext,R.raw.gameplay_car);
				}
				else
					M.loopSoundStop();
				mGR.gameStart();
...
//		Log.d(no, no);
	...
	void gameOver(int type)
	{
		...
		{
			M.sound2(GameRenderer.mContext,R.raw.crash);
			mGR.mPlayer.Strenth--;
			if(type == 0)
		...
		{
			M.loopSoundStop();
			 M.sound4(GameRenderer.mContext,R.raw.gameover);
	...
	void coinSet()
	{
		...
		for(i=0;i<mGR.mCoins.length;i++)
		{
			...
			if(CircRectsOverlap(mGR.mPlayer.x, mGR.mPlayer.y, mGR.mTex_Car[0].width()/4, mGR.mTex_Car[0].Height()/4, mGR.mCoins[i].x, mGR.mCoins[i].y, mGR.mTex_Coin[0].width()/2))
			{
				...
				Player.mTotalCoin++;
				M.sound6(GameRenderer.mContext,R.raw.coin);
				//break;
			}
			if(mGR.mCoins[i].y<-1)
				coin =i;
	...
	boolean HandleADD(MotionEvent event)
	{
		...
		if(event.getAction()==MotionEvent.ACTION_UP)
		{
			if(mGR.mSel==0)
			{
				...
				intent.setData(Uri.parse(M.LINK+"com.hututu.games.fightersofocean"));
				GameRenderer.mContext.startActivity(intent);//mTex_Rate
			}
			else
			if(mGR.mSel==1)
...
//		drawTex(gl, mGR.mTex_BackScr,0,0);
		...
		drawTex(gl, mGR.mTex_gLeft,-.840f,-.930f);//Start
	}
...
//			drawTex(gl, mGR.mTex_Menu[3],0.709f,-.610f);//About Us
		...
		if(mGR.mSel!=6)
			drawTex(gl, mGR.mTex_Menu[3],-.260f,-.800f);//Setting
...
//				M.GameScreen = M.GameSetting;
	...
	void DrawCarSelection(GL10 gl)
	{
		...
		drawTex(gl, mGR.mTex_Free[2],0.67f, 0.2f);
