...
public class GameRenderer implements Renderer , AccelerometerListener 
{
	...
	private Handler handlerR = new Handler() {public void handleMessage(Message msg) {mStart.adViewF.setVisibility(msg.what);}};
...
//			gameReset();
	...
	public void onDrawFrame(GL10 gl) 
	{
		...
		{
			resumeCounter++;
			if(resumeCounter>50 && !GameRenderer.mStart.addFree  &&(M.GameScreen !=  M.GameChallenge&& M.GameScreen != M.GamePause && M.GameScreen != M.GameOver &&  M.GameScreen != M.GameADD))
			{
		...
		if(mStart.adViewF!=null)
		{
			if(!GameRenderer.mStart.addFree &&(M.GameScreen == M.GamePause || /*M.GameScreen == M.GameOver||*/M.GameScreen ==  M.GameChallenge))
			{
				int inv=mStart.adViewF.getVisibility();
				if(inv==AdView.GONE){try{handlerR.sendEmptyMessage(AdView.VISIBLE);}catch(Exception e){}}
			}
			else
			{
				int inv=mStart.adViewF.getVisibility();
				if(inv==AdView.VISIBLE){try{handlerR.sendEmptyMessage(AdView.GONE);}catch(Exception e){}}
			}
		}
