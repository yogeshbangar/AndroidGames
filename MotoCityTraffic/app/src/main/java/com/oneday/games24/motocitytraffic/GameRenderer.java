...
public class GameRenderer implements Renderer {
    ...
    SimplePlane mTex_CoinIcn, mTex_PlayBtn, mTex_CtrlBtn, mTex_ChallengeBtn, mTex_Join, mTex_MenuBox, mTex_MenuIcn[], mTex_soundOff, mTex_MusicOff;
    SimplePlane mTex_PauseIcn[], mTex_AbtTxt, mTex_AchieveBtn, mTex_GamePower[], mTex_Lock, mTex_Power, mTex_Preview, mTex_SmallCoin, mTex_PreviewBox, mTex_UnlockTxt;
    ...
    SimplePlane mTex_SplashFont, mTex_AchieveUnlock, mTex_ChallengeBox, mTex_ChallengeRW[], mTex_ChallengeComp, mTex_LoadBar, mTex_Exit, mTex_Pointer;
    SimplePlane mTex_JoinText, mTex_JoinBtn, mTex_UpgradeFill, mTex_GamePopup, mTex_GameArrow, mTex_TapEffect, mTex_Buy,mTexFreeCoin;
    ...
    int mGameSec = 0, mGameMin = 1, mChallengeSec, noStage, mBestStage, mTotalMin, mTotalSec;
    ...
    boolean isAchieve[] = new boolean[50], isChallenge[] = new boolean[M.Challenge.length];
    int mNoCrossCar, mFreeCoin = 500;

    private Handler LoadAD = new Handler() {
        public void handleMessage(Message msg) {
            GameRenderer.mStart.loadInterstitial();
        }
    };
...
//			}
    ...
    void LoadUi() {
        ...
        mTex_ChallengeBtn = add("ui/challenge_button.png");
        mTex_CtrlBtn = add("ui/control_button.png");
        ...
        mTex_AchieveBtn = add("ui/avhievment_button.png");
        ...
        mTex_ChallengeRW = new SimplePlane[2];
        mTex_ChallengeRW[0] = add("ui/challenge_cross.png");
        mTex_ChallengeRW[1] = add("ui/challenge_right.png");
        mTex_ChallengeComp = add("ui/challenge_comp.png");
    ...
    int worldCoin[] = {0, 750, 1500, 5000, 5000, 5000, 5000, 7500, 7500, 7500, 7500, 10000};
    float worldDollar[] = {0, 0, 0, .99f, .99f, .99f, .99f, .99f, .99f, .99f, .99f, 1.99f};

    void setWorld() {
        mWorldSel = 0;
        for (int i = 0; i < mWorld.length; i++) {
            if (i < mWorld.length / 2) mWorld[i].set(0 + i * .4f, worldCoin[i], worldDollar[i]);
            else mWorld[i].set(0 - (i - 5) * .4f, worldCoin[i], worldDollar[i]);
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        gl.glClearColor(50 / 255f, 52 / 255f, 100 / 255f, 1);
        gl.glLoadIdentity();
        root.draw(gl);
        if (M.GameScreen != M.GAMEWORLD) gl.glDisable(GL10.GL_SCISSOR_TEST);
        //Banner AD
        resumeCounter++;
        if (resumeCounter > 500000) resumeCounter = 0;
        /*AdHouse*/
