...
public class Group extends Mesh {
    void DrawSetting(GL10 gl) {
        ...
        DrawTransScal(gl, mGR.mTex_Vibrate[M.setVibrator ? 1 : 0], -.11f, -.1f, mGR.mSel == 3 ? 1.5f : 1, mGR.mSel == 3 ? .4f : 1);
    ...
    boolean HandleSetting(MotionEvent event) {
        ...
        if (MotionEvent.ACTION_UP == event.getAction()) {
            ...
            if (mGR.mSel != 0)
                M.sound1(GameRenderer.mContext, R.raw.button);
            mGR.mSel = 0;
        }
    ...
    void DrawOverCom(GL10 gl) {
        ...
        if (M.GameScreen == M.GAMEWIN || M.GameScreen == M.GAMECONG) {
            ...
            if (mGR.mPlayer.accident < 3) {
                DrawTransScal(gl, mGR.mTex_LevelStar, -.205f, 0.01f, mGR.levelWinS1, 1);
                if (mGR.levelWinS1 >= 1) {
                    mGR.levelWinS1 *= .8f;
                    if (mGR.levelWinS1 <= 1) {
            ...
            if (mGR.mPlayer.accident < 2 && mGR.levelWinS1 < 1) {
                DrawTransScal(gl, mGR.mTex_LevelStar, -.005f, 0.09f, mGR.levelWinS2, 1);
                if (mGR.levelWinS2 >= 1) {
                    mGR.levelWinS2 *= .8f;
                    if (mGR.levelWinS2 <= 1) {
                        mGR.levelWinS2 = .98f;
            ...
            if (mGR.mPlayer.accident < 1 && mGR.levelWinS2 < 1) {
                DrawTransScal(gl, mGR.mTex_LevelStar, 0.205f, 0.015f, mGR.levelWinS3, 1);
                if (mGR.levelWinS3 >= 1) {
                    mGR.levelWinS3 *= .8f;
                    if (mGR.levelWinS3 <= 1) {
            ...
            switch (mGR.mSel) {
                ...
                case 5:
                    ...
                    if (M.GameScreen == M.GAMEPAUSE) {
                        ...
                        M.play(GameRenderer.mContext, R.raw.bg);
                    }
                    if (M.GameScreen == M.GAMEOVER) {
                        MoreGame();
                    }
    ...
    void GameWin() {
        ...
        if (mGR.unStar[mGR.mLevel - 1] < 3 - mGR.mPlayer.accident)
            mGR.unStar[mGR.mLevel - 1] = 3 - mGR.mPlayer.accident;
        ...
        M.playStop();
        M.sound4(GameRenderer.mContext, R.raw.lavelcomp);
    ...
    void life() {
        if (mGR.mPlayer.mBlast > 6) {
            ...
            M.sound2(GameRenderer.mContext, R.raw.carblast);
            if (mGR.mPlayer.accident < 3)
                mGR.mPlayer.accident++;
...
//						System.out.println("~~~~~~~~~~~~~~~~~Mukti~~~~~~~~~~~~~~~~");
        ...
        if (mGR.mPlayer.accident > 2 && mGR.mPlayer.mBlast > 5) {
            M.GameScreen = M.GAMEOVER;
            mGR.GameTime = System.currentTimeMillis() - mGR.GameTime;
            M.playStop();
            M.sound3Play(GameRenderer.mContext, R.raw.gameover);
            GameRenderer.mStart.showInterstitial();
        }
    ...
    void DrawGamePlay(GL10 gl) {
        ...
        DrawTextureR(gl, mGR.mTex_Wheel, x3, y3, mGR.mPlayer.a + mGR.mPlayer.pa);

        x4 = x1 + (float) Math.cos(Math.toRadians(mGR.mPlayer.a - 90)) * 0.06f * b;
        y4 = y1 + (float) Math.sin(Math.toRadians(mGR.mPlayer.a - 90)) * 0.10f * b;
        DrawTextureR(gl, mGR.mTex_Wheel, x4, y4, mGR.mPlayer.a + mGR.mPlayer.pa);
...
//		DrawTextureR(gl, mGR.mTex_Pahiye, x6,y6,mGR.mPlayer.a);
        ...
        DrawTextureRC(gl, mGR.mTex_Car[mGR.mPlayer.accident % 4], mGR.mPlayer.x, mGR.mPlayer.y, mGR.mPlayer.a, 1);
...
//			}
        ...
        DrawTexture(gl, mGR.mTex_Meter[(mGR.mPlayer.accident % 3) + 1], -.84f, 0.88f);
        ...
        DrawTransScal(gl, mGR.mTex_Paused, +.88f, 0.8f, mGR.mSel == 101 ? 1.5f : 1, mGR.mSel == 101 ? .4f : 1);

        mGR.mTex_String.drawTSR(gl, 0.8f, -.68f, mGR.mPlayer.angle != 0 ? 1.5f : 1, mGR.mPlayer.angle != 0 ? .6f : 1, mGR.mPlayer.pa * 8);
        mGR.mTex_Speed.drawTSR(gl, -.8f, -.2f, mGR.mPlayer.speed == 1 ? 1.5f : 1, mGR.mPlayer.speed == 1 ? .4f : 1, 000);
        mGR.mTex_Speed.drawTSR(gl, -.8f, -.8f, mGR.mPlayer.speed == 2 ? 1.5f : 1, mGR.mPlayer.speed == 2 ? .4f : 1, 180);
        gameLogic();
    }
...
//        GameRenderer.mContext.startActivity(mIntent);
    ...
    void drawAnimation(GL10 gl) {
        for (int i = 0; i < mGR.ani.length; i++) {
            if (mGR.ani[i].x > -1.2 && mGR.ani[i].x < 1.2 && mGR.ani[i].y > -1.2 && mGR.ani[i].y < 1.2) {
                mGR.mTex_8x8.drawRGBS(gl, mGR.ani[i].x, mGR.ani[i].y, mGR.ani[i].r, mGR.ani[i].g, mGR.ani[i].b, mGR.ani[i].scale);
                mGR.ani[i].update();
            }
        }
    }
    ...
    public boolean Handle(MotionEvent event) {
        if (CircRectsOverlap(-.8f, 0.0f, .1f, .1f, screen2worldX(event.getX()), screen2worldY(event.getY()), .1f))
            GameRenderer.mStart._keyCode = 3;
        if (CircRectsOverlap(0.8f, 0.0f, .1f, .1f, screen2worldX(event.getX()), screen2worldY(event.getY()), .1f))
            GameRenderer.mStart._keyCode = 4;
        if (CircRectsOverlap(-.0f, -.8f, .1f, .1f, screen2worldX(event.getX()), screen2worldY(event.getY()), .1f))
            GameRenderer.mStart._keyCode = 1;
        if (CircRectsOverlap(0.0f, 0.8f, .1f, .1f, screen2worldX(event.getX()), screen2worldY(event.getY()), .1f))
...
//                    M.GameScreen = M.GAMEADD;//AdHouse
            ...
            case M.GAMEADD:
                if (Counter > 100)
                    ...
                    DrawTexture(gl, mGR.mTex_HighBar, 0.2f, 0.9f);
                    DrawTexture(gl, mGR.mTex_Pointer, .2f + Counter * (mGR.mTex_HighBar.width() / 100f) - .32f, 0.9f);
                }
                break;
            /*AdHouse*/
            case M.GAMEMENU:
...
//        DrawTransScal(gl, mGR.mTex_GPPlus, -.88f, -.8f, mGR.mSel == 4 ? 1.5f : 1, mGR.mSel == 4 ? .4f : 1);
