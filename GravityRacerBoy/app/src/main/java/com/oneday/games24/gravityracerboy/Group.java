		...
		for (int i = 0; i < mGR.mTile.length; i++) {
			...
			if (mGR.mTile[i].x - .2 < mGR.mGuy.x&& mGR.mTile[i].x + .2 > mGR.mGuy.x){
				...
				for (int j = 0; j < mGR.mTile[i].array.length; j++) {
					if (mGR.mTile[i].array[j] != 0) {
						if (isIn(mGR.mTile[i].array[j]) == 0)
						{
			...
			if(mGR.mTile[i].x < -1.05f){
				...
				for (int j = 0; j < mGR.mL01.l1.length; j++) {
					if (mGR.mGuy.colNo >= mGR.mL01.l1[0].length) {
						...
						mGR.mTile[i].array[j] = (short)(9+((i%2)+j)%2);
					} else {
						switch (mGR.Levelno) {
						case 13:
							mGR.mTile[i].array[j] = mGR.mL13.l1[j][mGR.mGuy.colNo];
							break;
						case 12:
							mGR.mTile[i].array[j] = mGR.mL12.l1[j][mGR.mGuy.colNo];
							break;
						case 11:
							mGR.mTile[i].array[j] = mGR.mL11.l1[j][mGR.mGuy.colNo];
							break;
						case 10:
							mGR.mTile[i].array[j] = mGR.mL10.l1[j][mGR.mGuy.colNo];
							break;
						case 9:
							mGR.mTile[i].array[j] = mGR.mL09.l1[j][mGR.mGuy.colNo];
							break;
						case 8:
							mGR.mTile[i].array[j] = mGR.mL08.l1[j][mGR.mGuy.colNo];
							break;
						case 7:
							mGR.mTile[i].array[j] = mGR.mL07.l1[j][mGR.mGuy.colNo];
							break;
						case 6:
							mGR.mTile[i].array[j] = mGR.mL06.l1[j][mGR.mGuy.colNo];
							break;
						case 5:
							mGR.mTile[i].array[j] = mGR.mL05.l1[j][mGR.mGuy.colNo];
							break;
						case 4:
							mGR.mTile[i].array[j] = mGR.mL04.l1[j][mGR.mGuy.colNo];
							break;
						case 3:
							mGR.mTile[i].array[j] = mGR.mL03.l1[j][mGR.mGuy.colNo];
							break;
						case 2:
							mGR.mTile[i].array[j] = mGR.mL02.l1[j][mGR.mGuy.colNo];
							break;
						case 1:
						default:
							mGR.mTile[i].array[j] = mGR.mL01.l1[j][mGR.mGuy.colNo];
							break;
						}
					}
				}
...
//				M.sound2(R.raw.game_over);
	...
	void Draw_Play(GL10 gl){
		...
		for(int i =0;i<mGR.mTile.length;i++){
			for(int j = 0; j<mGR.mTile[i].array.length; j++){
				if(mGR.mTile[i].array[j] >0){
					if (isIn(mGR.mTile[i].array[j]) != 2)
						DrawTexture(gl, mGR.mTex_Tile[mGR.mTile[i].array[j]-1], mGR.mTile[i].x,.93f - j * mGR.mTex_Tile[i].Height());
				}
			}
		}
